 ...
 * 获取消息控制类.
 */
public class CtSYSC320001GetMessage extends BusiService {
    
    /** The CLAS s_ name. */
    private static String CLASS_NAME = "SYSC320001";
    ...
    private int getPara(IParameterObj param, IResultObj result) {
        try {
            ...
            if (mParam.get("receive_man") == null || mParam.get("receive_man").toString().length() == 0) {
                errorHandle(result, INFO, CLASS_NAME + "0104", "入参查询人姓名不能为空");
                return -1;
            }
//            if (mParam.containsKey("hosp_flag") && (mParam.get("hosp_flag") != null || mParam.get("hosp_flag").toString().length() != 0)) {
//                this.strHospFlag = mParam.get("hosp_flag").toString();
//            }
     ...
     * @return 大于0表示成功，小于0表示出错
     */
    public int getMessageInfo(IResultObj result) {
        int iReturn = 0;

        try {
        	BulletinDAO dao = getBean(BulletinDAO.class);
        	
        	List lst = dao.getQueryMessage(this.strCenterId, this.strOrg, this.strStaff);
        	iReturn = CollectionHelper.size(lst);
            if (iReturn > 0) {
            	for(int i = 0; i < iReturn; i ++){
            		Map m = (Map) lst.get(i);
            		m.put("ms_content", formatMessage(UtilFunc.getString(m, "ms_content")));
            	}
            ...
            errorHandle(result, 5, getActionName() + "获取消息信息时出现异常", ex);
            return FAILURE;
        }
    }
    ...
    public static String formatMessage(String msg){
    	if(UtilFunc.isEmpty(msg)){
    		return "";
    	}
    	
    	String ret = msg;
    	
    	ret = UtilFunc.replace(ret, "\r", "");
    	ret = UtilFunc.replace(ret, "\n", "`^`");
    	ret = UtilFunc.replace(ret, "\t", "`@`");
