...
(function() {
	...
	(function() {
		...
		var winVisible = function(visible) {
			...
			//curCfg.message="<a target='home' onclick='closeWind()' href='http://localhost:8080/wf_hxz/inform/ViewInform.action?infoId="+curCfg.message+"'>"+curCfg.title+"</a><br><a target='home' href='/wf_hxz/inform/queryReceiveInformList.action'>[查看更多]</a>";
				
			//curCfg.message=curCfg.message+"<br><a target='home' href='http://wwww.baidu.com'>baidu</a>";
			
			//alert(curCfg.message);
			
			ym_body.innerHTML = !curCfg.iframe ? ('<div class="' + curCfg.msgCls + '" style=\'margin-top:-20px;margin-left:20px;\'>' + curCfg.message + '</div>') : "<iframe style='" + ifmStyle + "' border='0' frameborder='0' src='" + curCfg.message + "'></iframe><div style='" + ifmStyle + ";background:#000;opacity:0.1;filter:alpha(opacity=10);display:none'></div>"; //内容
			(function(el, obj) {
...
function closeWind(){
	ymPrompt.close();
	parent.addTab("home","消息浏览页面","<%=path%>/pages/welcome.jsp");
}
