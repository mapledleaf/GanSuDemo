...
<powersi:html>
<powersi:head title="公告审签" />
</head>
<body>
	<powersi:panelbox key="panel_query" allowAddition="true">
		<powersi:form id="queryForm" namespace="/message" action="BulletinManagerAction!querySecondAuditList">
	...
	<div class="hidden">
		<div id="dlg">
			<div class="textLeft"><span class="textInfo"><i class="icon-info-sign"></i> 请填写审签意见</span></div>
				<div style="margin: 5px 0 15px 0;">
					<powersi:textarea id="opinion" name="opinion" rows="5" maxlength="500"></powersi:textarea>
				</div>
				<div class="textRight">
					<powersi:button id="btAudit" onclick="saveAudit(true)" value="审签通过" cssClass="btn btn-success" buttonIcon="icon-check" />
					<powersi:button id="btNotPass" onclick="saveAudit(false)" value="审签不通过" cssClass="btn btn-danger" buttonIcon="icon-ban-circle" />
...
<script type="text/javascript">
	function gridRowRender(rowdata, rowid){
		if(rowdata['second_audit_flag'] == '2') {
			return getColorStyle('error');
		}
		
		if(rowdata['second_audit_flag'] == '1') {
	...
	function renderOperate(row, index, value){
		var a = [];
		a.push('<input type="button" value="审签公告" class="linkButton"');
		a.push(' onclick="doAudit(');
		a.push(index);
		a.push(')"');
		if(row['second_audit_flag'] == '1'){
		...
		a.push('<input type="button" value="取消审签" class="linkButton"');
		a.push(' onclick="doCancel(');
		a.push(index);
		a.push(')"');
		if(row['second_audit_flag'] == '0'){
			a.push(' disabled="disabled"');
		}
		a.push(" />");
	...
	function renderReply2(rowdata, index, value) {
  	...
  	function doEdit(index) {
		...
		popupDialog({
			url: rootPath + "/message/BulletinEditAction.action?auditType=2&bulletinId=" + bulletinId,
			onClosed: function() {
  	...
  	function doAudit(index) {
  		...
  		if(dlg){
  			...
  			dlg = popupDiv('#dlg', '审签公告', 350);
  		}
  		
  		$('#opinion').val(row['second_audit_opinion'] || '').focus();
	}
  	...
  	function saveAudit(flag){
  		...
  		if (!confirm("您确认审签" + (flag ? "通过" : "不通过") + "公告[" + bulletinTitle + "]吗?")) {
  			return;
  		}
  		
  		postJSON(rootPath + "/message/BulletinManagerAction!secondAudit.action", {
  	...
  	function doCancel(index) {
  		...
  		if (!confirm("您确认取消审签公告[" + bulletinTitle + "]吗?")) {
            return;
        }
  		
  		postJSON(rootPath + "/message/BulletinManagerAction!cancelSecondAudit.action?bulletinId=" + bulletinId, function(json){
  			if(!checkJSONResult(json)){
			    return;
		    }
