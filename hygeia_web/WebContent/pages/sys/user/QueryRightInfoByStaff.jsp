...
<body>
	...
	<powersi:panelbox key="panel_result" title="用户列表">
		<powersi:datagrid id="grid" formId="queryForm" 
			...
			<powersi:toolbar>
				<powersi:toolbar-item id="export" text="导出用户权限列表" icon="export" click="exportInfo" />
			</powersi:toolbar>
...
<script type="text/javascript">
	...
	function gridDblClickRow(rowdata, rowindex, rowobj) {
		viewRight(rowdata['staff_id']);
	}
	
	function viewRight(id){
		popupDialog(rootPath + "/manager/MenuManager!showMenuTree.action?staff_id="+id, 550, 500);
	}
	...
	function exportInfo(){
		try{
			...
			postDownload(rootPath + "/user/UserRightAction!exportInfo.action", {
				'staffId': powersi.tostring(rows)
			});
		} catch(e){
			alert(e.message);
		}
