...
<body>
	...
	<powersi:form id="referralTransferAuditid" namespace="/particular"
		action="ParticularManagerAction!referralTransferAudit.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>	
				...
				<powersi:button id="btreferralTransferAuditReset" label="重置" 
				    onclick="referralTransferAuditReset()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
				    ...
				    <powersi:radio id="bka600" name="inHospitalDTO.bka600" label="转入标识"
	...
	<powersi:panelbox key="业务列表">
		<powersi:datagrid id="grid" formId="referralTransferAuditid"
	...
	<script type="text/javascript">
		var queryReferralTransferAuditFlag = false;
		var objCard = null;
		...
		function readic() {
			...
			$("#referralTransferAuditid").submit();
			$("#bke548").val("");
		}

		/*重置界面*/
		function referralTransferAuditReset() {
			var aac003 = powersi.trim($("#aac003").val());
			if (!powersi.isnull(aac003) && !queryReferralTransferAuditFlag) {
		...
		function renderOperate(row, index, value) {
			...
			a.push('<input type="button" value="审批通过" class="linkButton"');
			a.push(' onclick="doCheckPass(');
			a.push(index);
			a.push(')"');
			...
			a.push('<input type="button" value="审批不通过" class="linkButton"');
			a.push(' onclick="doCheckNotPass(');
			a.push(index);
			a.push(')"');
			if (row['bke058'] == '1' || row['bke058'] == '2') {
				a.push(' disabled="disabled"');
		...
		//审批通过
		function doCheckPass(index){
			var row = grid.getRow(index);
			var aaz267 = row['aaz267'];
			popupDialog({
				url:"${rootPath}/particular/ParticularManagerAction!referralTransferInfoCheckPass.action?inHospitalDTO.aaz267="+aaz267,
				onClosed:function(){
					$("#referralTransferAuditid").submit();
				}
			},screen.height,screen.width);
		...
		//审批不通过
		function doCheckNotPass(index){
			var row = grid.getRow(index);
			var aaz267 = row['aaz267'];
			popupDialog({
				url:"${rootPath}/particular/ParticularManagerAction!referralTransferInfoCheckNotPass.action?inHospitalDTO.aaz267="+aaz267,
				onClosed:function(){
					$("#referralTransferAuditid").submit();
				}
			},screen.height,screen.width);
