...
<powersi:codetable id="ake003List" codeType="ake003" codeFilter="data_value in(1)" />
	...
	<powersi:tabbedpanel id="divTabs">
		...
		<div id="divTab1">
			<powersi:datagrid id="matchList" title="提示：勾选后点击删除按钮删除" url="${rootPath }/medicare/HospCataAction!queryMatchCata.action"
				...
				<powersi:datagrid-column name="aae100" label="有效标志"  width="70" render="renderAae100"/>
		...
		<div id="divTab2" class="row">
			<div class="col-6">
				<powersi:datagrid id="hospCata" title="医院待匹配药品目录" url="${rootPath }/medicare/HospCataAction!queryHospCata.action"
					...
					<powersi:datagrid-column name="ake003" label="目录类别" code="ake003List" />
					<powersi:datagrid-column name="bkc140" label="单价" />
					<powersi:datagrid-column name="bkm017" label="药监本位码" />
					<powersi:datagrid-column name="bkm005" label="药品剂型"  />
			...
			<div class="col-6">
				<powersi:datagrid id="centerCata" title="中心可匹配药品目录" url="${rootPath }/medicare/HospCataAction!queryCenterWaitCata.action"
					...
					<powersi:datagrid-column name="ake003" label="目录类别" code="ake003List" />
					<powersi:datagrid-column name="aka065" label="收费等级" render="renderAka065" />
...
<script type="text/javascript">
    /* /
      修改概要：TS19081500020 - 【需求开发】药品目录匹配界面里面的一些输入框太短了，输入的信息完全看不见，希望把输入框调整长一些
     修改描述：屏蔽掉输入框限制长度的地方，让他随框架自适应
	...
	function onDblClickRow_hospCata(data, rowid, rowdata) {
		$("input[ligeruiid=centerCata_ake006]").val("");
		$("input[ligeruiid=centerCata_bkm017]").val(data.bkm017);
		if(data.bkm017==""){
...
// 				return;
	...
	function renderAka065(rowdata, index, value){
		if(value=="1") return "甲类";
		else if(value=="2") return "乙类";
		else if(value=="3") return "全自费"; 
		else return value;
	}
