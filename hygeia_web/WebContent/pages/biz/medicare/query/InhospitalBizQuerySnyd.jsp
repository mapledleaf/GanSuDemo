...
<powersi:html>
<head>
<powersi:head title="省内异地就诊信息查询" />
</head>
<body>
	<powersi:form id="queryForm" namespace="/query"
		action="BizQueryAction!queryHospitalInfo.action">
		<powersi:panelbox key="panel_query" title="查询条件">
			...
			<powersi:editorlayout cols="10%,15%,8%,12%,8%,12%,9%,7%,13%">
				<tr>
					...
					<td colspan="2" class="tdLabel"><powersi:textfield
							id="arg_value" placeholder="请输入信息回车.."
							name="bizQueryDto.arg_value" onkeydown="queryByPersonInfo()"/></td>
					<td align="right" class="tdLabel">在院状态</td>
					<td><select id="zys001" name="bizQueryDto.zys001" class=""
				...
				<tr>
					...
					<td><select id="aka130" name="bizQueryDto.aka130" >
							<option value="72">省内异地</option>
						</select></td>
					<td colspan="2"></td>
					<td align="right" class="tdLabel">结算状态</td>
					<td><select id="zys002" name="bizQueryDto.zys002"
							onchange="settleStatus()">
							<option value="1">未结算</option>
							<option value="2">已结算</option>
						</select></td>
					<td align="right"><input type="radio" id="endclinicDate"
						name="radio_date" value="rad_todate"onchange="changeDatetype()"/>
						<label id="settleDate">结算时间</label></td>
	...
	<script type="text/javascript">
		...
		function onSelectClick(index) {
			...
			if (one == '1') {
				postJSON(
						"${rootPath}/query/BizQueryAction!queryCenterBizFeeInfo.action?bizQueryDto.aaz217="
								+ aaz217 + "&bizQueryDto.akb020=" + akb020,
						afterInHospBizFee);
			}
		}
		...
		function feeDetailTable() {
			...
			popupDialog(
					{
						url : "${rootPath}/pages/biz/medicare/query/InhospitalBizQueryFeeDetailSnyd.jsp?bizQueryDto.akb020="
								+ akb020
								+ "&bizQueryDto.aaz217="
								+ aaz217
								+ "&bizQueryDto.secfalg="
