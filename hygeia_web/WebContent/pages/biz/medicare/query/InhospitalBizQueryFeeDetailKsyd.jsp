...
<powersi:form id="feeDetailForm" action="BizQueryAction!queryFeeDetailTable.action" namespace="/query">
	...
	<powersi:groupbox title="费用明细清单">
		<powersi:datagrid id="feeDetailGrid" height="410" delayLoad="true"
			totalRender="feeDetailList" showExportExcel="true"
			showExportExcel2007="true" showExportPdf="true"  formId="feeDetailForm"
...
<script type="text/javascript">
		window.onload = function(){
			...
			/*if(one == '1'){
				//自动查询装载进入datagrid(也可以用postJSON)
				feeDetailGrid.set("url", "${rootPath}/query/BizQueryAction!queryKsydFeeDetailTable.action?bizQueryDto.akb020="+akb020+
						"&bizQueryDto.aaz217="+aaz217); 
			}else{
				//自动查询装载进入datagrid(也可以用postJSON)
				feeDetailGrid.set("url", "${rootPath}/query/BizQueryAction!queryFeeDetailTable.action?bizQueryDto.akb020="+akb020+
		...
		function btn_fun() {
			...
			/*if (one == '1') {
				postJSON(
						"${rootPath}/query/BizQueryAction!queryKsydFeeDetailTable.action?bizQueryDto.aaz217=" + aaz217 + 
								"&bizQueryDto.fromdate="+ fromdate+ 
								"&bizQueryDto.todate=" + todate,
						afterInHospBizFee);
			} else {
				postJSON(
						"${rootPath}/query/BizQueryAction!queryFeeDetailTable.action?bizQueryDto.aaz217="+ aaz217
								+ "&bizQueryDto.secfalg=" + secfalg
								+ "&bizQueryDto.fromdate=" + fromdate
								+ "&bizQueryDto.todate=" + todate,
						afterInHospBizFee);
