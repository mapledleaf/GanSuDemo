...
<body>
		...
		<powersi:panelbox key="查询条件">
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					...
					<powersi:hidden id="allowed_no_readcard" key="是否必须读卡"
						name="diagnoseInfoDTO.allowed_no_readcard" value="1" />
				</powersi:editorlayout-row>
			</powersi:editorlayout>
		</powersi:panelbox>
		<tags:mediBizInfo_diagnose_charge_healthposts />
		<tags:mediFeeInfo_datagrid_healthposts />
		<tags:mediHisPayInfo />
	...
	<script type="text/javascript">
		...
		function resetpage() {
			...
			$("#reduceflag").val("0");
			$("input[type='radio'][name=operFlag][value='1']").attr("checked",
					true);
			...
			popupDialog(
					{
						url : "${rootPath}/diagnose/GetPersonInfoAction!getPersonBusi.action?diagnoseInfoDTO.arg_name="
								...
								+ $("#aka130").val()
								+ "&diagnoseInfoDTO.allowed_no_readcard=1",
						onClosed : function() {
							...
							if (ret != null) {
								...
								postJSON(
										...
										function(json) {
											...
											$("#aac006").val(
													json.data[0].aac006);
											$("[name='diagnoseInfoDTO.bka004']")
													.val(json.data[0].bka004);
											$("[name='diagnoseInfoDTO.baa027']")
													.val(json.data[0].baa027);
											$("#bka008").val(
													json.data[0].bka008);
											$("#aab001").val(
													json.data[0].aab001);
											...
											if (operFlag == "1") {
												...
												if (json.data[0].bka001 > 1) {
													...
													popupDialog(
															{
																url : "${rootPath}/pages/biz/medicare/diagnose/chooseDiagnoseFee.jsp?aaz217="
																		+ $(
																				"#aaz217")
																				.val()
																		+ "&kcd1id="
																...
																onClosed : function() {
																	...
																	if (ret != null) {
																		...
																		postJSON(
																				"${rootPath}/diagnose/GetPersonInfoAction!getBusiFeeInfo.action?aaz217="
																						+ $(
																								"#aaz217")
																								.val()
																						+ "&kcd1id="
