...
<powersi:html>
<head>
<powersi:head title="卫生站就诊信息批量导入" />
</head>
<body>
	...
	<div id="paneltab1">
	  <powersi:datagrid id="kf01Grid" 
				...
				<powersi:datagrid-column name="oper" display="操作" width="170" render="renderOper"/>
				...
				<powersi:datagrid-column name="bkc032" display="导入时间"  width="130" format="dateFmt:'yyyy-MM-dd HH:mm:ss'" />
				<powersi:datagrid-column name="bkc033" display="文件名称"  width="200" />
				<powersi:datagrid-column name="bkc034" display="文件类型"  width="55" />
				<powersi:datagrid-column name="bkc040" display="导入状态标志"  width="110" render="renderStatus"/>
				<powersi:datagrid-column name="progress" display="校验进度" width="75" render="renderProgress"/>
	  </powersi:datagrid> 
	
	</div>
	<div id="paneltab2">
	   <powersi:datagrid id="kf02Grid" enabledExportExcel="true" exportFileName="'卫生站批量费用导入校验结果'">
	...
	function renderOper(rowdata, index, value){
	    ...
	    if(rowdata.progress!='100'){
	    	oper=oper+"&nbsp;&nbsp;"+
		       "<a href='javascript:void(0)' onclick='velidDate("+rowdata.kf01id+")'>重新校验</a>";
	    }
		return oper;
	}
	
	function showStatusDate(rowdata, index ,status){
		alert(rowdata+"="+index+"="+status);
	}
	...
	var velidDate=function(kf01id){
		var url="${rootPath}/diagnose/DiagnoseBatchChargeAction!velidDate.action?kf01DTO.kf01id="+kf01id;
		var params=form2json("#batchSeacherForm");
		postJSON(url, params, function(json){
			queryData(0);
