	...
	<powersi:form id="bizForm" method="post">
		<powersi:panelbox key="查询条件" allowCollapse="false">
			<powersi:panelbox-toolbar>
				...
				<powersi:button id="btnCalc1" key="button_calc1" onclick="calcReadic(1);" disabled="false" />
		...
		<tags:mediBizInfo_drug_charge />
		<tags:mediFeeInfo_datagrid_drug />
		<tags:mediHisPayInfo/>
		<powersi:hidden id="bka006_erx" name="bka006_erx" />
		<powersi:hidden id="bke548" name="inHospitalDTO.bke548" />
		<powersi:hidden id="bka018" name="diagnoseInfoDTO.bka018" />
...
<!--  【NTS20050800118】电子凭证动态库开发 电子凭证接口请求js by 钟声 20200505 -->
 <script type="text/javascript" src="${rootPath}/resource/js/medicaretag/electronicVoucher.js" charset="UTF-8"></script>
	...
	function getPerson(para) {
			...
			postJSON(
					...
					function(json) {
						...
						if (json.errortype == '0') {
							if(json.data.personinfo.aae140=='390'){
								popupInfo("城乡居民未开通购药业务!");
								$("#querystring").attr("disabled", false);
								init();
								return;
							}
							/* if(json.data.personinfo.aae140=='340'){
								popupInfo("离休统筹药店均未开通!");
								return;
							} */
						...
						if (ret != null) {
							...
							postJSON(
									"${rootPath}/medicare/GetPersonInfoAction!getPersonBusiDetail.action?akb020="
											+akb020+"&aaz217="+aaz217+"&aka130="+aka130+"&bka006="+bka006,
									function(json) {
										if (!checkJSONResult(json)) {
											return;
										}
