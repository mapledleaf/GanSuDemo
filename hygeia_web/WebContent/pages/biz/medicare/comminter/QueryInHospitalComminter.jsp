...
<body>
	...
	<powersi:panelbox key="业务列表">
		<powersi:datagrid id="grid" formId="queryInHospitalForm"
			...
			<powersi:datagrid-column name="bka507" display="审核状态"
				code="audit_flag" width="80" />
			...
			<powersi:datagrid-column name="aae140" display="险种类型" code="aae140"
				width="150" />
			<powersi:datagrid-column name="bka510" display="审核说明" width="150" />
		</powersi:datagrid>
	</powersi:panelbox>
	<powersi:errors />
	<script type="text/javascript">
		...
		function doCancelOutCharge(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的出院结算吗?")) {
				return;
			}
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!cancelOutCharge.action?inHospitalDTO.kc21id="
							+ kc21id + "&inHospitalDTO.aaz217=" + aaz217+"&flag=1&inHospitalDTO.comm_subsys_flag=1",
					function(json) {
						if (!checkJSONResult(json)) {
							return;
						}
						var messageArray =  json.message;
						popupInfo(messageArray[0]);
						grid.deleteRow(index);
						if(messageArray.length > 0){
							popupInfo(json.data[1].bill);
						}
					});
		}
		...
		function doCancelRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的住院登记吗?")) {
				return;
			}
			//读卡验证
			var bke548 = null;// 读卡返回
