...
<powersi:html>
<powersi:head title="门诊数据查询" />
<%@ include file="/pages/util/header/combogrid.jsp"%>
	<powersi:form id="bizForm" method="post" action="/universal/DiagnoseQueryAction_HIS!queryOutpatientPersonInfo_page.action">
		<powersi:panelbox key="查询条件">
			...
			<%-- <powersi:editorlayout cols="8%,8%,8%,8%,15%,5%">
			
				<powersi:editorlayout-row>
				  <powersi:codeselect id="caa027" key="caa027" name="inHospitalDTO.caa027" codeType="caa027" required="true" headerKey="0" />
				<powersi:textfield id="outpatient_time" name="" mask="date" key="就医登记时间"  
			...
			<powersi:editorlayout cols="6">
				...
				<td class="tdInput"><select id="bka044" name="kcd1_Mz_HisDTO.bka044"  class="select" onchange="">
						<option value="0">未上传</option>
						<!-- <option value="1">未结算</option>
						<option value="2">已结算 </option> -->
						<!-- <option value="3">重传 </option> -->
				</select>
				<powersi:textfield id="akc190" name="kcd1_Mz_HisDTO.akc190"  key="门诊号"  
					onKeyDown="" onchange="" required="false" />
				<powersi:textfield id="outpatient_time" name="kcd1_Mz_HisDTO.aae036" mask="date" key="就医登记时间" format="dateFmt:'yyyy-MM-dd'" 
					onKeyDown="" onchange="" required="false" />
				</powersi:editorlayout-row>
			</powersi:editorlayout>
		</powersi:panelbox>
	...
	<div class="col-5">
		<powersi:panelbox key="panel_result" title="门诊人员信息" >
			<powersi:datagrid id="grid" formId="bizForm" name="Kcd2_HospFee_HisDTOList" height="100%"  width="100%" enabledSort="true" delayLoad="true" checkbox="false" onSelectRow="queryFeeInfo">
	       			<powersi:datagrid-column name="aac003" display="姓名" align="center" />
	       			<powersi:datagrid-column name="akc190" display="门诊号" align="center" />
	       			<powersi:datagrid-column name="kcd1id" display="门诊序列号" align="center" />
	...
	<!-- 费用明细 -->
	<powersi:form id="bizForm2" method="post" action="/universal/DiagnoseQueryAction_HIS!queryOutpatientPersonFee_page.action">
		<%-- <powersi:submit style="display:none;" id="btSaveRegisterhidden" label="业务查询" onclick="" /> --%>
		<powersi:hidden id="bkm902hidden" name="kcd1_Mz_HisDTO.bkm902" value=""></powersi:hidden>
	</powersi:form>
	<div class="col-7">
...
function feeUpload(){
	//打开窗口，通过住院号
	var url = "${rootPath}/pages/biz/medicare/diagnose_his/chargeUpload_dialog.jsp";
	var dlg = popupDialog({
		url: url,
		onClosed: function(){
...
function queryOnePersonInfo(){
	...
	var url = "${rootPath}/diagnose_his/DiagnoseQueryAction_HIS_HIS!queryOutpatientPersonInfo.action";
	//获取结算状态
	var bka044 = $("#bka044").val();
	//业务类型
	//var businessType = $("#businessType").val();
	//业务类型的值
	//var bnTypeValue = $("#bnTypeValue").val();
	//就医登记时间
	var outpatient_time = $("#outpatient_time").val();
	//门诊号
	var bkm902 = $("#bkm902").val();
	//url = url+"?bka044="+bka044+"&"+businessType+"="+bnTypeValue+"&aae036="+outpatient_time
	//url = url + "?bka044="+bka044+"&bkm902="+bkm902+"&aae036="+outpatient_time
	postJSON(url,
		{
		bka044:bka044,
		bkm902:bkm902,
		aae036:outpatient_time
		},
  		importCalcInfo);
}
...
function queryFeeInfo(data,rowid,rowdata){
	gridFee.reset();
	$("#bkm902hidden").val(data.bkm902);
	$("#bizForm2").submit();
	//var url = "${rootPath}/diagnose_his/DiagnoseQueryAction_HIS_HIS!queryOutpatientPersonFee.action";
...
function feeUploadAgain(){
	...
	if(powersi.isempty(data)){
		popupAlert("请选择一行需要上传的门诊信息");
		return;
	}
	var url = "${rootPath}/universal/DiagnoseQueryAction_HIS!outpatientfeeUploadAgain.action";
...
//门诊登记信息抓取
function getRegInfo(){
	var url = "${rootPath}/pages/biz/medicare/diagnose_his/outpatient_dialog.jsp";
	var dlg = popupDialog({
		url: url,
		onClosed: function(){
			//var ret = this.returnValue;
