...
<powersi:html>
<head>
<powersi:head title="医院体检目录修改" target="_self" />
</head>
<body>
	<powersi:form id="mainform" disabled="true">
		<powersi:editorlayout cols="10%,20%,15%,15%,15%,15%">
			...
			<powersi:editorlayout-row>
				...
				<powersi:textfield id="bkh027" name="healthDTO.bkh027"
					label="体检项目编码" required="true" validate="maxSize[20]" />
				<powersi:textfield id="bkh028" name="healthDTO.bkh028"
					label="体检项目名称" required="true" validate="maxSize[100]" />
				<powersi:codeselect id="bkh046" name="healthDTO.bkh046" key="bkh046"
					label="统计类别" codeType="bkh046" cssClass="select2"  headerKey="-1"
					required="true"  />
			...
			<powersi:editorlayout-row>
				...
				<powersi:codeselect id="bkh048" name="healthDTO.bkh048" key="bkh048"
					label="是否离休体检项目" list="#{'1':'不是','2':'是'}" headerKey="-1"
					required="true" />
			...
			<powersi:editorlayout-row>
				...
				<powersi:hidden id="bke207" name="healthDTO.bke207" label="审核时间"/>
				<powersi:hidden id="bke208" name="healthDTO.bke208" label="审核人工号" />
				<powersi:hidden id="bke209" name="healthDTO.bke209" label="审核人" />
				<powersi:hidden id="aae016" name="healthDTO.aae016" label="审核标志" />
				<powersi:hidden id="bkh047" name="healthDTO.bkh047" label="版本号" />
				<powersi:hidden id="aae100" name="healthDTO.aae100" label="有效标志" code="aae100"/>
				<powersi:hidden id="bka044" name="healthDTO.bka044" label="上传标志"  value="0"/>
				<powersi:hidden id="flag" name="healthDTO.flag" label="删除"  value="1"/>
			</powersi:editorlayout-row>
	...
	window.onload = function(){
		currItemData=$("#mainform").serialize();
		var myDate = new Date();
	...
	function save() {
		...
		if(currItemData == saveItemData){
			popupAlert("未修改任何数据！");
			return;
		}
		postJSON("${rootPath}/medicare/HealthAction!updateHospCata.action", saveItemData, function(json) {
			if (!checkJSONResult(json)) {
				return;
			}
			//alert(json.data);
			popupAlert(json.data, "提示", "info");
			/* if (json.data == '改修目录信息成功！') {
				closeDialog();
			} */
		});
