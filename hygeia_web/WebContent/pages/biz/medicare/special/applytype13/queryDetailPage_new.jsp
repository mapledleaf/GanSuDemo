...
<powersi:html>
	...
	<powersi:form id="mainForm" action="specialManager!getQueryDetailPage.action" namespace="/medicarespecial">
		...
		<powersi:groupbox title="上传文件信息（单击展开）" isCollapse="true">
		...
		<powersi:groupbox allowCollapse="false">
			<powersi:editorlayout>
				<tr>
					<td colspan="5" style="text-align: left">
						<label for="aae017">医院审核意见:&nbsp;&nbsp;</label>
						<input type="text" id="aae017" name="aae017" style="width:90%;"
							<%if("1".equals(request.getAttribute("dto.aae016"))){%>
							   disabled="disabled"
							<%}%>
						/>
					</td>
				...
				<tr>
					<td colspan="6" style="text-align: center">
						<powersi:button onclick="goConfirm('0')" value="同 意" key="button_yes"/>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<powersi:button onclick="goConfirm('1')" value="不同意" key="button_no"/>
					</td>
				</tr>
				<%}%>
			</powersi:editorlayout>
			...
			<powersi:codetable id="aae016List" codeType="aae016_spapp"/>
			<powersi:codetable id="aae100List" codeType="aae100"/>
		</powersi:groupbox>
		<powersi:errors />
	</powersi:form>

	<div id="showImgMask" style="display:none;"></div>
	<div id="showImg" style="display:none;text-align:center"></div>
	</body>
...
<script type="text/javascript">
	...
	function queryAllImg(){
		postJSON("${path}!queryAllImg.action?dto.aaz267="+$("#aaz267").val(), function(json){
			if (!checkJSONResult(json)) {
			...
			$.each(json.data, function(key, value){
				...
				for(var i = 0; i< imgList.length; i++){
					...
					showDiv.append("<img src='" + imgSrc + "' class='photo' style='height: " + pheight + "px;width: " + pwidth + "px' onclick='zoomIn(this)'/>");
					showDiv.append("&nbsp;&nbsp;</td>");
				}
			});
		});
	...
	function zoomIn(obj) {
		var showImg = $("#showImg");
		showImg.append("<img src='" + obj.src + "' class='photo' style='height: " + 9*pheight + "px;width: " + 9*pwidth + "px' onclick='zoomOut(this)'/>");
	...
	function goConfirm(flag) {
		var aae017 = $("#aae017").val();
		if ('1' === flag && $.trim(aae017) === "") {
			popupAlert("请输入审核不通过意见！");
		} else {
			postJSON("${path}!updateKcc6.action", {
						"dto.aae013": aae017, "dto.aaz267": $("#aaz267").val()
						, "dto.aae016": flag
					},
					function (json) {
						if (!checkJSONResult(json)) {
