	...
	<powersi:form id="mainForm" action="specialManager!getSpeAppInfList.action" namespace="/medicarespecial">
		<powersi:panelbox title="查询条件">
			...
			<div style="display: none">
				...
				<powersi:textfield id="aka083" name="dto.aka083" key="aka083" value="${dto.aka083}" label="申请类型"/>
				...
				<powersi:textfield id="comFlag" name="dto.comFlag" label="特殊标志：101定点医院修改"/>
				<powersi:textfield id="bka100" name="dto.bka100" key="bka100" label="社会保障号"/>
			</div>
			
			<powersi:editorlayout cols="8">
				<tr>
					<powersi:codeselect id="argName" name ="dto.argName" list="%{#{'aac002':'社会保障号','aac003':'姓名','aac001':'个人电脑号'}}" label="查询类型" />
				...
				<tr>
					<% if(null != request.getAttribute("dto.comFlag")&&"101".equals(request.getAttribute("dto.comFlag").toString())){ %>
						<powersi:codeselect id="audit_status" name ="dto.audit_status" list="%{#{'7':'全部','0':'未审核','1':'审核通过'}}" label="审核状态" />
					<%}else{ %>
...
<script type="text/javascript">
	...
	function queryAll() { 
		...
		if(argName == 'aac002'){
			...
			if (queryString != null && queryString != "") {
				postJSON("${path}!getPersonSize.action", {"aac002":queryString}, function(result) {
					if (result.errortype == 0) {
						var returnData = result.data;
						var rowsize = returnData.rowsize;
						var persinfo = returnData.persinfo;
						if (rowsize > 0) {
							if (rowsize == 1) {
								$("#querystring").val(persinfo.aac001);
								$("#argName").val("aac001");
								query();
							} else {
								// 弹出选人窗口b
								queryUrl = "${path}!getPersonList.action?aac002=" + queryString;
								popupDialog(queryUrl, 600, 800, function(ret) {
									if (ret) {
										returnData = powersi.tojson(ret);
										$("#querystring").val(returnData.aac001);
										$("#argName").val("aac001");
										query();
									}
								});
	...
	function query(){
		if($("#aae127_end").val()!="" && $("#aae127_start").val()!="" ){
			if($("#aae127_end").val()<$("#aae127_start").val()){popupAlert('结束时间不能小于开始时间！');return;}
		}
		if($("#aac001").val() == ''){
	...
	function renderOperate1(row, index, value) {
		try {
			...
			if($("#comFlag").val() == "101") {
				a.push(' <input title="单击进入定点医院修改页面" type="button" value="择点" class="linkButton"');
				a.push(' onclick="getModifyDDPage(');
				a.push('\'' + row.aaz267 + '\'');
				a.push(',\'' + row.aka083 + '\'');
				a.push(')"');
				a.push(" />");
				
				a.push(' <input title="单击进入并发症修改页面" type="button" value="并发症" class="linkButton"');
				a.push(' onclick="getModifyDiseasePage(');
				a.push('\'' + row.aaz267 + '\'');
				a.push(',\'' + row.aka083 + '\'');
				a.push(')"');
				a.push(" />");
	...
	//修改页面
	function getModifyDDPage(aaz267, aka083) {
		var param = "dto.aae140=" + $("#aae140").val() + "&dto.pageFlag=" + getPageInitParamByApplyType(aka083, 'pageFlag') + "&dto.aaz267=" + aaz267 
		+ "&dto.aaa027=" + $("#aaa027").val() + "&dto.caa027=" +$("#caa027").val() + "&dto.auditType=" + $("#auditType").val()+ "&dto.comFlag=" + $("#comFlag").val();
		popupDialogWithParam({
	...
	//并发症
	function getModifyDiseasePage(aaz267, aka083){
		var param = "dto.aae140=" + $("#aae140").val() + "&dto.pageFlag=" + getPageInitParamByApplyType(aka083, 'pageFlag') + "&dto.aaz267=" + aaz267 
		+ "&dto.aaa027=" + $("#aaa027").val() + "&dto.caa027=" +$("#caa027").val() + "&dto.auditType=" + $("#auditType").val()+ "&dto.comFlag=102";
		popupDialogWithParam({
			url: "${path}!getApplyModifyPage.action?" + param,
			onClosed: function(){
			}
