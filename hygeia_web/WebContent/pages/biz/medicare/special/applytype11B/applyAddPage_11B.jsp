...
<body>
	<powersi:form id="mainForm" action="specialManager!addSpeAppInf.action" namespace="/medicarespecial">
		...
		<div style="display: none">
			...
			<powersi:textfield id="aka083" name="dto.aka083" key="aka083" value="11B" label="申请类型"/>
			<powersi:textfield id="aaa027" name="dto.aaa027" key="aaa027" value="<%= aaa027  %>"  label="统筹区编码"/>
			<powersi:textfield id="aka130" name="dto.aka130" key="aka130" value="11" label="业务类型"/>
			<powersi:textfield id="bka006" name="dto.bka006" key="bka006" value="110" label="待遇类型"/>
			...
			<powersi:editorlayout cols="8">
				<tr>
					<powersi:textfield id="aka121" name="dto.aka121" required="true" readonly="true"
						buttonId="di" onbuttonclick="chooseDis('mzlb',aka120,aka121)" label="申请疾病" />
				...
				<tr>
					<powersi:textfield id="bke017" name="dto.bke017" key="bke017" label="申请医师" required="true"/>
...
<script type="text/javascript">
	...
	function saveinfo() {
		...
		if ($("#aae140").val()!='390'){
			popupAlert("非城乡居民不能进行门诊两病备案!", "提示", "error");
			return false;
		}
		
		if ($("#hosplist").val()==null||$("#hosplist").val()==""){
			$("#hosplist").val($("#akb020").val()+',');
		}
	...
	function initForm() {
		...
		//获取失效日期 默认一年
		var dt = new Date();
		var endDate = dt.getFullYear() + 1;
		if (dt.getMonth() + 1 <10) endDate += "0";
		endDate += '' + (dt.getMonth()+1);
		if (dt.getDate() <10) endDate += "0";
		endDate += '' + dt.getDate();
		$("#aae031").val(endDate);
	}
	...
	*/
     var tabKeyConf=["akb021:1","aae030:1","aae031:1","bke033","bke001",
 	            	"aka121:1","aae019","bke015","bke014","bke017",
 	            	"bke020","bke018","bke019","bke011:0,1","aae013:0,1"];
	//配置特殊的事件
