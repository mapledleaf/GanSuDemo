...
<%
	...
	String aka083Filter = "aaa102 not in ('166','180','200','201','202','203')";
	...
	<powersi:form id="mainForm" action="specialManager!getSpeAppInfList.action" namespace="/medicarespecial">
		<powersi:panelbox title="查询条件">
			...
			<div style="display: none">
				...
				<powersi:textfield id="aka083" name="dto.aka083" key="aka083" value="131"/>
			</div>
			...
			<powersi:editorlayout cols="8">
				...
				<tr>
					<powersi:codeselect id="hosp_audit_status" name ="dto.hosp_audit_status" list="%{#{'all':'全部','0':'未审核','1':'审核通过','2':'审核未通过'}}" label="医院审核状态" />
					<powersi:codeselect id="audit_status" name ="dto.audit_status" list="%{#{'':'全部','0':'未审核','1':'审核通过','2':'审核未通过'}}" label="中心审核状态" />
					<td colspan="2"/>
				</tr>
		...
		<powersi:groupbox title="查询结果">
			...
			<powersi:datagrid id="grid1" formId="mainForm" 
				...
				<powersi:datagrid-column display="操作" frozen="true" key="operate" render="renderOperate" width="10%"/>
				...
				<powersi:datagrid-column name="aae127" display="申请日期" width="7%"/>
				<powersi:datagrid-column name="aae030" display="生效日期" width="7%"/>
				<powersi:datagrid-column name="aae031" display="失效日期" width="7%"/>
				<powersi:datagrid-column name="hosp_audit_status" display="医院审核状态" render="renderDictionaryStatic" isExport="false"  isSort="false" width="8%"/>
				<powersi:datagrid-column name="aae016" display="中心审核状态" render="renderDictionaryStatic" isExport="false"  isSort="false" width="8%"/>
				<powersi:datagrid-column name="aaz267" display="申请号" hide="true" isExport="false"  width=""/>
			</powersi:datagrid>
			<powersi:codetable id="aka083List" codeType="aka083"/>
...
<script type="text/javascript">
	...
	function queryAll() {
		if ($("#aae127_end").val() < $("#aae127_start").val()) {
			popupAlert('结束时间不能小于开始时间！');
			return;
		}
		$("#" + $("#argName").val()).val(powersi.trim($("#querystring").val()));
		grid1.reset();
	...
	function renderOperate(row, index, value) {
		try {
			var a = [];
			if(row.aae016 === '1'){
				a.push('<input title="单击可查看" type="button" value="查看" class="linkbutton"');
				a.push(' onclick="getDetailPage(');
			}else{
				a.push('<input title="单击可审核" type="button" value="审核" class="linkbutton"');
				a.push(' onclick="getDetailPage(');
			}
			...
			a.push('<input title="单击可删除" type="button" value="删除" class="linkbutton"');
			a.push(' onclick="deleteApply(');
			a.push('\'' + row.aaz267 + '\'');
			a.push(')"');
			if(row.aae016 === '1'){
	...
	//查看详细信息
	function getDetailPage(aaz267, aka083, aae016) {
		var param = "dto.aae140=" + $("#aae140").val()
				+ "&dto.pageFlag=" + getPageInitParamByApplyType(aka083, "pageFlag")
				+ "&dto.aaz267=" + aaz267
				+ "&dto.aaa027=" + $("#aaa027").val()
				+ "&dto.comFlag=21"
				+ "&dto.caa027=" +$("#caa027").val()
				+ "&dto.aae016=" + aae016
				+ "&dto.auditType=1"
				+ "&dto.kcc601=1";
	...
	function getModifyPage(aaz267, aka083) {
		...
		+ "&dto.aaa027=" + $("#aaa027").val() + "&dto.comFlag=21" + "&dto.caa027=" +$("#caa027").val();
		popupDialogWithParam({
