	...
	<powersi:form id="mainForm" action="specialManager!getSpeAppInfList.action" namespace="/medicarespecial">
		<powersi:panelbox title="查询条件">
			...
			<div style="display: none">
				...
				<powersi:textfield id="comFlag" name="dto.comFlag" key="comFlag"  label="特殊标志：102：可以修改复审之后的数据"/>
			</div>
			...
			<powersi:editorlayout cols="8">
				...
				<tr>
					<powersi:codeselect id="audit_status" name ="dto.audit_status" list="%{#{'':'全部','0':'未审核','3':'初审通过','4':'初审未通过','1':'复审通过','2':'复审未通过'}}" label="审核状态" /><td colspan="6"/>
					<td colspan="6"/>
				</tr>
...
<script type="text/javascript">
	...
	function renderOperate1(row, index, value) {
		try {
			...
			a.push(" />");
			if($("#comFlag").val() == "102") {
				a.push(' <input title="单击进入修改页面" type="button" value="修改" class="linkButton"');
	...
	function getModifyPage(aaz267, aka083) {
		var param = "dto.aae140=" + $("#aae140").val() + "&dto.pageFlag=" + getPageInitParamByApplyType(aka083, 'pageFlag') + "&dto.aaz267=" + aaz267 + "&dto.comFlag=" + $("#comFlag").val()
