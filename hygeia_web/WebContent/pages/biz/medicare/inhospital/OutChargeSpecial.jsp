...
<body>
	<powersi:form id="outChargeSpecial" namespace="/inhospital"
		action="InhospitalManagerAction!outChargeSpecial.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outChargeSpecialTryCalcTreat()" />
				<powersi:button id="btCalcOutCharge" label="结算收费"
					onclick="saveOutCharge()" />
				<powersi:button id="btoutChargeSpecialPrint" label="打印结算单"
					onclick="outChargeSpecialPrint()" />
				<powersi:button id="btoutChargeSpecialReset" label="重置"
					onclick="outChargeSpecialReset()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findOutChargeaaz217Special()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$("#querystring").focus();
			$("#btoutChargeSpecialPrint").prop("disabled", true);
		...
		/*读卡获取后台信息*/
		function readic() {
			outChargeSpecialReset();
			iReadCardBase();
			if (powersi.isnull($("#bke548").val())) {
				return;
			}
			findOutChargeaaz217SpecialByBka100();
		}

		/*查询就医登记信息*/
		function findOutChargeaaz217SpecialByBka100() {
			var outChargeData = $("#outChargeSpecial").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findOutChargeaaz217Special.action",
		...
		/*费用计算(试算)*/
		function outChargeSpecialTryCalcTreat() {
			...
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outChargeSpecialTryCalcTreat.action?inHospitalDTO.kcd1id="
		...
		function saveOutCharge() {
			...
			feeItemReset();
			$("#btCalcOutCharge").prop("disabled", true);
			var dataOutCharge = $("#outChargeSpecial").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outChargeSpecial.action",
		...
		function afterSaveOutCharge() {
			...
			$("#btoutChargeSpecialPrint").prop("disabled", false);
			$("#btTryCalcTreat").prop("disabled", true);
			$("#btCalcOutCharge").prop("disabled", true);
		}
		...
		/*重置界面*/
		function outChargeSpecialReset() {
			$("#btReset").click();
			feeItemReset();
			$("#aaz217_hid").val('');
			$("#kcd1id_hid").val('');
		...
		/*打印结算单*/
		function outChargeSpecialPrint() {
		...
		/*查询就医登记信息*/
		function findOutChargeaaz217Special() {
			if (window.event.keyCode == 13) {
				$("#bke548").val("");
				$("#bke549").val("");
				$("#pcardinfo").val("");
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findOutChargeaaz217Special.action?inHospitalDTO.querystring="
