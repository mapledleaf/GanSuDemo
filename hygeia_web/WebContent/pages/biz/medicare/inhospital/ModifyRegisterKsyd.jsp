...
<powersi:html>
<head>
<powersi:head title="跨省异地就医住院信息修改" />
</head>
<body>
	<powersi:form id="modifyRegisterKsyd" namespace="/inhospital"
		action="InhospitalManagerAction!modifyRegisterKsyd.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btSaveModifyRegister" label="保存修改"
					onclick="saveModifyRegisterKsyd()" />
				<powersi:button id="btModifyRegisterReset" label="重置"
					onclick="modifyRegisterKsydReset()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findModifyRegisteraaz217Ksyd()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		/*读卡获取后台信息*/
		function readic() {
			modifyRegisterKsydReset();
			iReadCardBase();
			if (powersi.isnull($("#bke548").val())) {
				return;
			}
			findModifyRegisteraaz217KsydByBka100();
		}

		function findModifyRegisteraaz217KsydByBka100() {
			var modifyRegisterData = $("#modifyRegisterKsyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findModifyRegisteraaz217Ksyd.action",
		...
		function saveModifyRegisterKsyd() {
			if (!checkForm()) {
				return;
			}
			...
			var modifyRegisterKsydData = $("#modifyRegisterKsyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!modifyRegisterKsyd.action",
					modifyRegisterKsydData, function(json) {
		...
		function modifyRegisterKsydReset() {
			$("#btReset").click();
			//入院科室
			$("#bka019").val("");
			//入院病区
		...
		function findModifyRegisteraaz217Ksyd() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findModifyRegisteraaz217Ksyd.action?inHospitalDTO.querystring="
