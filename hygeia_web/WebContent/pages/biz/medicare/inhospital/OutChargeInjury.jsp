...
<body>
	<powersi:form id="outChargeInjury" namespace="/inhospital"
		action="InhospitalManagerAction!outChargeInjury.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outChargeInjuryTryCalcTreat()" />
				<powersi:button id="btCalcOutCharge" label="结算收费"
					onclick="saveOutChargeInjury()" />
				<powersi:button id="btoutChargeInjuryPrint" label="打印结算单"
					onclick="outChargeInjuryPrint()" />
				<powersi:button id="btoutChargeInjuryReset" label="重置"
					onclick="outChargeInjuryReset()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findOutChargeaaz217Injury()" buttonText="读卡"
	...
	<powersi:panelbox cssStyle="display:none;">
		<powersi:form id="fundformInjury" namespace="/inhospital"
	...
	<powersi:panelbox key="基金支付">
		<powersi:datagrid id="grid" formId="fundformInjury" height="85%"
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#btoutChargeInjuryPrint").prop("disabled", true);
		...
		/*读卡获取后台信息*/
		function readic() {
			outChargeInjuryReset();
			iReadCardBase();
			if (powersi.isnull($("#bke548").val())) {
				return;
			}
			findOutChargeaaz217InjuryByBka100();
		}

		/*查询就医登记信息*/
		function findOutChargeaaz217InjuryByBka100() {
			var outChargeData = $("#outChargeInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findOutChargeaaz217Injury.action",
		...
		/*费用计算(试算)*/
		function outChargeInjuryTryCalcTreat() {
			...
			$("#btTryCalcTreat").prop("disabled", true);
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outChargeInjuryTryCalcTreat.action?inHospitalDTO.kcd1id="
		...
		function saveOutChargeInjury() {
			...
			feeItemReset();
			$("#btCalcOutCharge").prop("disabled", true);
			var dataOutCharge = $("#outChargeInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outChargeInjury.action",
		...
		function afterSaveOutCharge() {
			...
			$("#btoutChargeInjuryPrint").prop("disabled", false);
			$("#btTryCalcTreat").prop("disabled", true);
			$("#btCalcOutCharge").prop("disabled", true);
		}
		...
		function queryFundPay(bka891) {
			...
			$("#fundformInjury").submit();
		}

		/*重置界面*/
		function outChargeInjuryReset() {
			$("#btReset").click();
			feeItemReset();
			$("#aaz217_hid").val('');
			$("#kcd1id_hid").val('');
		...
		/*查询就医登记信息*/
		function findOutChargeaaz217Injury() {
			if (window.event.keyCode == 13) {
				$("#bke548").val("");
				$("#bke549").val("");
				$("#pcardinfo").val("");
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findOutChargeaaz217Injury.action?inHospitalDTO.querystring="
