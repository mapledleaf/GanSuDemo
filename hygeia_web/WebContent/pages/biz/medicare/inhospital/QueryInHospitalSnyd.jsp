...
<powersi:html>
<head>
<powersi:head title="省内异地住院就医回退" />
</head>
<body>
	<powersi:form id="queryInHospitalSnyd" namespace="/inhospital"
		action="InhospitalManagerAction!queryInHospitalSnyd.action">
	...
	<powersi:panelbox key="业务列表">
		<powersi:datagrid id="grid" formId="queryInHospitalSnyd"
	...
	<script type="text/javascript">
		...
		function renderOperate(row, index, value) {
			var a = [];
			a.push('<input type="button" value="取消省内异地住院" class="linkButton"');
			...
			a.push('<input type="button" value="取消省内异地出院" class="linkButton"');
			...
			a.push('<input type="button" value="取消省内异地结算" class="linkButton"');
			...
			a.push('<input type="button" value="取消省内异地中途结算" class="linkButton"');
		...
		function doCancelOutCharge(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的省内异地出院结算吗?")) {
				return;
			}
			postJSON(
		...
		function doCancelRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的省内异地住院登记吗?")) {
				return;
			}
			//读卡验证
			loadCardControl();
