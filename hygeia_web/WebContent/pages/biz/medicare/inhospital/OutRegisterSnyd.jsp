...
<body>
	<powersi:form id="outRegisterSnyd" namespace="/inhospital"
		action="InhospitalManagerAction!outRegisterSnyd.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				...
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outRegisterSnydTryCalcTreat()" />
				<powersi:button id="btSaveOutRegister" label="保存登记"
					onclick="saveOutRegisterSnyd()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findOutRegisteraaz217Snyd()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		function readic() {
			...
			findOutRegisteraaz217SnydByBka100();
		}

		function findOutRegisteraaz217SnydByBka100() {
			var outRegisterData = $("#outRegisterSnyd").serialize();
			postJSON(
		...
		/*查询在院登记业务信息*/
		function findOutRegisteraaz217Snyd() {
			if (window.event.keyCode == 13) {
				$("#bke548").val("");
		...
		/*费用计算(试算)*/
		function outRegisterSnydTryCalcTreat() {
			if (!checkForm()) {
				return;
			}
			...
			feeItemReset();
			var dataOutRegister = $("#outRegisterSnyd").serialize();
			$("#btTryCalcTreat").prop("disabled", true);
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterSnydTryCalcTreat.action",
		...
		/*保存省内异地就医出院*/
		function saveOutRegisterSnyd() {
			if (!checkForm()) {
				return;
			}
			var aac003 = powersi.trim($("#aac003").val());
			if (!confirm("您确认保存[" + aac003 + "]的省内异地就医出院登记吗?")) {
				return;
			}
			$("#btSaveOutRegister").prop("disabled", true);
			var dataOutRegister = $("#outRegisterSnyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterSnyd.action",
					dataOutRegister, function(json) {
						$("#btSaveOutRegister").prop("disabled", false);
						if (!checkJSONResult(json)) {
							return;
		...
		function outRegisterSnydInputFee() {
			...
			var outRegisterSnydData = $("#outRegisterSnyd").serialize();
			postJSON(
