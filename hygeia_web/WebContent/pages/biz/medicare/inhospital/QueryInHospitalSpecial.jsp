...
<powersi:html>
<head>
<powersi:head title="门特就医回退" />
</head>
<body>
	<powersi:form id="queryInHospitalSpecial" namespace="/inhospital"
		action="InhospitalManagerAction!queryInHospitalSpecial.action">
	...
	<powersi:panelbox key="业务列表">
		<powersi:datagrid id="grid" formId="queryInHospitalSpecial"
	...
	<script type="text/javascript">
		...
		function renderOperate(row, index, value) {
			var a = [];
			a.push('<input type="button" value="取消门特登记" class="linkButton"');
			...
			a.push('<input type="button" value="取消门特出院" class="linkButton"');
			...
			a.push('<input type="button" value="取消门特结算" class="linkButton"');
		...
		function doCancelOutCharge(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的门特出院结算吗?")) {
				return;
			}
			postJSON(
		...
		function doCancelOutRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的门特出院登记吗?")) {
				return;
			}
			postJSON(
		...
		function doCancelRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的门特入院登记吗?")) {
				return;
			}
			//读卡验证
			loadCardControl();
