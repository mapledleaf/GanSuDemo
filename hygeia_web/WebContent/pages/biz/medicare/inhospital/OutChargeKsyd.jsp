...
<body>
	<powersi:form id="outChargeKsyd" namespace="/inhospital"
		action="InhospitalManagerAction!outChargeKsyd.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outChargeKsydTryCalcTreat()" />
				<powersi:button id="btCalcOutCharge" label="结算收费"
					onclick="saveOutChargeKsyd()" />
				<powersi:button id="btoutChargeKsydPrint" label="打印结算单"
					onclick="outChargeKsydPrint()" />
				<powersi:button id="btoutChargeKsydReset" label="重置"
					onclick="outChargeKsydReset()" />
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$("#querystring").focus();
			$("#btoutChargeKsydPrint").prop("disabled", true);
		...
		function findOutChargeaaz217KsydByBka100() {
			var outChargeData = $("#outChargeKsyd").serialize();
			postJSON(
		...
		/*费用计算(试算)*/
		function outChargeKsydTryCalcTreat() {
			...
			$("#btTryCalcTreat").prop("disabled", true);
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outChargeKsydTryCalcTreat.action?inHospitalDTO.kcd1id="
		...
		function saveOutChargeKsyd() {
			...
			feeItemReset();
			$("#btCalcOutCharge").prop("disabled", true);
			var dataOutCharge = $("#outChargeKsyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outChargeKsyd.action",
		...
		function afterSaveOutCharge() {
			...
			$("#btoutChargeKsydPrint").prop("disabled", false);
			$("#btTryCalcTreat").prop("disabled", true);
			$("#btCalcOutCharge").prop("disabled", true);
		}
