...
<powersi:html>
<head>
<powersi:head title="省内异地就医住院修改" />
</head>
<body>
	<powersi:form id="modifyRegisterSnyd" namespace="/inhospital"
		action="InhospitalManagerAction!modifyRegisterSnyd.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btSaveModifyRegister" label="保存修改"
					onclick="saveModifyRegisterSnyd()" />
				<powersi:button id="btModifyRegisterReset" label="重置"
					onclick="modifyRegisterSnydReset()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findModifyRegisteraaz217Snyd()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		/*读卡获取后台信息*/
		function readic() {
			modifyRegisterSnydReset();
			iReadCardBase();
			if (powersi.isnull($("#bke548").val())) {
				return;
			}
			findModifyRegisteraaz217SnydByBka100();
		}

		function findModifyRegisteraaz217SnydByBka100() {
			var modifyRegisterData = $("#modifyRegisterSnyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findModifyRegisteraaz217Snyd.action",
		...
		function saveModifyRegisterSnyd() {
			if (!checkForm()) {
				return;
			}
			...
			var modifyRegisterSnydData = $("#modifyRegisterSnyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!modifyRegisterSnyd.action",
		...
		function modifyRegisterSnydReset() {
			$("#btReset").click();
			//入院科室
			$("#akf001").val("");
			//入院病区
		...
		function findModifyRegisteraaz217Snyd() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findModifyRegisteraaz217Snyd.action?inHospitalDTO.querystring="
