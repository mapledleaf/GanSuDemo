...
<body>
	<powersi:form id="modifyRegisterSpecial" namespace="/inhospital"
		action="InhospitalManagerAction!modifyRegisterSpecial.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btSaveModifyRegister" label="保存修改"
					onclick="saveModifyRegister()" />
				<powersi:button id="btModifyRegisterReset" label="重置"
					onclick="modifyRegisterSpecialReset()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findModifyRegisteraaz217Special()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		/*读卡获取后台信息*/
		function readic() {
			modifyRegisterSpecialReset();
			iReadCardBase();
			if (powersi.isnull($("#bke548").val())) {
				return;
			}
			findModifyRegisteraaz217SpecialByBka100();
		}

		function findModifyRegisteraaz217SpecialByBka100() {
			var modifyRegisterData = $("#modifyRegisterSpecial").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findModifyRegisteraaz217Special.action",
		...
		function saveModifyRegister() {
			...
			var modifyRegisterSpecialData = $("#modifyRegisterSpecial")
					.serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!modifyRegisterSpecial.action",
					modifyRegisterSpecialData, function(json) {
		...
		function modifyRegisterSpecialReset() {
			$("#btReset").click();
			//入院科室
			$("#bka019").val("");
			//入院病区
		...
		function findModifyRegisteraaz217Special() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findModifyRegisteraaz217Special.action?inHospitalDTO.querystring="
