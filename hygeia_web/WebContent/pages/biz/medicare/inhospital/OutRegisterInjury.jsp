...
<body>
	<powersi:form id="outRegisterInjury" namespace="/inhospital"
		action="InhospitalManagerAction!outRegisterInjury.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outRegisterInjuryTryCalcTreat()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findOutRegisteraaz217Injury()" buttonText="读卡"
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				...
				<powersi:editorlayout-row>
					...
					<powersi:textfield id="bka018" name="inHospitalDTO.bka018"
						label="多诊断" title="请输入出院多诊断" placeholder="请输入入院多诊断"
	...
	<script type="text/javascript">
		...
		function readic() {
			...
			findOutRegisteraaz217InjuryByBka100();
		}

		function findOutRegisteraaz217InjuryByBka100() {
			var outRegisterData = $("#outRegisterInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findOutRegisteraaz217Injury.action",
		...
		/*查询在院登记业务信息*/
		function findOutRegisteraaz217Injury() {
			if (window.event.keyCode == 13) {
				$("#bke548").val("");
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findOutRegisteraaz217Injury.action?inHospitalDTO.querystring="
		...
		/*费用计算(试算)*/
		function outRegisterInjuryTryCalcTreat() {
			if (!checkForm()) {
				return;
			}
			...
			var dataOutRegister = $("#outRegisterInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterInjuryTryCalcTreat.action",
		...
		function saveOutRegister() {
			...
			var dataOutRegister = $("#outRegisterInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterInjury.action",
					dataOutRegister, function(json) {
						$("#btSaveOutRegister").prop("disabled", false);
						if (!checkJSONResult(json)) {
							return;
