...
<powersi:html>
<head>
<powersi:head title="跨省异地住院就医回退" />
</head>
<body>
	<powersi:form id="queryInHospitalKsyd" namespace="/inhospital"
		action="InhospitalManagerAction!queryInHospitalKsyd.action">
	...
	<powersi:panelbox key="业务列表">
		<powersi:datagrid id="grid" formId="queryInHospitalKsyd"
	...
	<script type="text/javascript">
		...
		function renderOperate(row, index, value) {
			var a = [];
			a.push('<input type="button" value="取消跨省异地住院" class="linkButton"');
			...
			a.push('<input type="button" value="取消跨省异地出院" class="linkButton"');
			...
			a.push('<input type="button" value="取消跨省异地结算" class="linkButton"');
		...
		function doCancelOutCharge(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的跨省异地出院结算吗?")) {
				return;
			}
			postJSON(
		...
		function doCancelOutRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的跨省异地出院登记吗?")) {
				return;
			}
			postJSON(
		...
		function doCancelRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的跨省异地住院登记吗?")) {
				return;
			}
			//读卡验证
			loadCardControl();
