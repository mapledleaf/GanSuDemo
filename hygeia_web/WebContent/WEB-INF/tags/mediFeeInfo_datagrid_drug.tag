...
<script type="text/javascript">
	...
	var actionUrl = "${rootPath}/common/CommonManagerAction!chooseHospitalCatalogMatched.action?searchType=py&ake007="
	   + curDateStr+"&type=2"+"&inHospitalDTO.caa027="+$("#caa027").val();
	...
	function insertfee() {
		...
		var jsonFee = {
			...
			"bka040" : bka040,
			"akc226" : akc226,
			"aae019" : parseFloat(input_ver).toFixed(4) * 1.0000,
			"ake007" : ake007,
			"bkc106" : bkc106,
			"aka057" : aka057,
			"ake001" : ake001,
		...
		if (window.event.keyCode == '13') {
			...
			if ('akc226' == filed_name) {
				...
				if (!powersi.isnull(akc226) && !powersi.isnull(bka040)) {
					var aae019 = (parseFloat(akc226) * parseFloat(bka040)).toFixed(2);
					$("#aae019").val(aae019);
				}
				return $("#aae019").focus();
			}
...
<powersi:panelbox key="费用明细信息" allowCollapse="false">
	...
	<powersi:editorlayout cols="5%,7%,1%,13%,5%,17%,3%,9%,3%,9%,3%,9%,5%,11%" >
	...
	<powersi:datagrid id="gridFeeDetail" height="93%" delayLoad="true" onAfterAddRow="scrollToRowLast"
		showReload="false" autoWidth="true" alternatingRow="true" checkbox="true" 
		colDraggable="false" usePager="false" totalRender="renderTotal">
