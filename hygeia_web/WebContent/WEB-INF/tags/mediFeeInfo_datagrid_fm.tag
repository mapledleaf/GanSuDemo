...
<script type="text/javascript">
	...
	function insertfee() {
		...
		var bka040 = parseFloat($("#bka040").val()).toFixed(2);
		...
		if (window.event.keyCode == '13') {
			...
			if ('akc226' == filed_name) {
				...
				if (!powersi.isnull(akc226) && !powersi.isnull(bka040)) {
					$("#aae019").val(parseFloat(akc226) * parseFloat(bka040));
				}
				return $("#aae019").focus();
			}
...
<powersi:panelbox key="费用明细信息">
	...
	<powersi:datagrid id="gridFeeDetail" height="100%" delayLoad="true" onAfterAddRow="scrollToRowLast"
