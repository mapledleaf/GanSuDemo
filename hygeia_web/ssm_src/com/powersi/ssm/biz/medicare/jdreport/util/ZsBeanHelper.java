package com.powersi.ssm.biz.medicare.jdreport.util;
...
public class ZsBeanHelper {
		...
		if (source instanceof Map) {
			...
			for (Iterator iterator = entrySet.iterator(); iterator.hasNext();) {
				try {
					...
					ZsPropertyHelper.setProperty(target, entry.getKey(), entry.getValue(), ignoreNull);
				} catch (Exception e) {
			...
			for (int i = 0; i < sourceProperties.length; i++) {
				try {
					...
					ZsPropertyHelper.setProperty(target, sourcefieldName, sourcefieldValue, ignoreNull);
				} catch (Exception e) {
					System.out.println("warn:" + e.getMessage());
				}
			}
	...
	public static <T> T getBean(Class<T> clazz) {
		return com.powersi.hygeia.framework.util.BeanHelper.getBean(clazz);
	}
				...
				try {
					...
					for (String cname : PropertyName) {
						if (entry.getKey().equals(cname)) {
							ZsPropertyHelper.setProperty(target, entry.getKey(), entry.getValue(), false);
						}
					}
				} catch (Exception e) {
					System.out.println("warn:" + e.getMessage());
				...
				try {
					...
					for (String cname : PropertyName) {
						if (sourcefieldName.equals(cname)) {
							...
							Object sourcefieldValue = null;
							sourcefieldValue = PropertyUtils.getProperty(source, sourcefieldName);
							ZsPropertyHelper.setProperty(target, sourcefieldName, sourcefieldValue, false);
						}
					}
				} catch (Exception e) {
					System.out.println("warn:" + e.getMessage());
