...
<body>
	<powersi:form id="outRegisterKsyd" namespace="/inhospital"
		action="InhospitalManagerAction!outRegisterKsyd.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btSaveFee" label="上传费用"
					onclick="outRegisterKsydInputFee()" />
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outRegisterKsydTryCalcTreat()" />
				<powersi:button id="btSaveOutRegister" label="保存登记"
					onclick="saveOutRegisterKsyd()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findOutRegisteraaz217Ksyd()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		function readic() {
			...
			findOutRegisteraaz217KsydByBka100();
		}

		function findOutRegisteraaz217KsydByBka100() {
			var outRegisterData = $("#outRegisterKsyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findOutRegisteraaz217Ksyd.action",
		...
		/*查询在院登记业务信息*/
		function findOutRegisteraaz217Ksyd() {
			if (window.event.keyCode == 13) {
				$("#bke548").val("");
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findOutRegisteraaz217Ksyd.action?inHospitalDTO.querystring="
		...
		/*费用计算(试算)*/
		function outRegisterKsydTryCalcTreat() {
			if (!checkForm()) {
				return;
			}
			...
			feeItemReset();
			var dataOutRegister = $("#outRegisterKsyd").serialize();
			$("#btTryCalcTreat").prop("disabled", true);
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterKsydTryCalcTreat.action",
		...
		/*保存跨省异地就医出院登记*/
		function saveOutRegisterKsyd() {
			if (!checkForm()) {
				return;
			}
			var aac003 = powersi.trim($("#aac003").val());
			if (!confirm("您确认保存[" + aac003 + "]的跨省异地就医出院登记吗?")) {
				return;
			}
			$("#btSaveOutRegister").prop("disabled", true);
			var dataOutRegister = $("#outRegisterKsyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterKsyd.action",
					dataOutRegister, function(json) {
						$("#btSaveOutRegister").prop("disabled", false);
						if (!checkJSONResult(json)) {
							return;
		...
		function outRegisterKsydInputFee() {
			var kcd1id = powersi.trim($("#kcd1id").val());
			if (powersi.isnull(kcd1id)) {
				return;
			}
			...
			var outRegisterKsydData = $("#outRegisterKsyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterKsydInputFee.action",
					outRegisterKsydData, function(json) {
