...
<body>
	<powersi:form id="mainForm" action="specialManager!addSpeAppInf.action" namespace="/medicarespecial">
		...
		<div style="display: none">
			...
			<powersi:textfield id="aka083" name="dto.aka083" key="aka083" value="206" label="申请类型"/>
			...
			<powersi:textfield id="aae030" name="dto.aae030" key="aae030" label="开始日期"/>
			<powersi:textfield id="aae031" name="dto.aae031" key="aae031" label="结束日期"/>
		...
		<powersi:groupbox title="住院信息">
			<powersi:editorlayout cols="8">
				<tr>
					...
					<powersi:textfield id="bke053" name="dto.bke053"  readonly="true" label="就诊医院"/>
					<powersi:hidden id="aaz286" name="dto.aaz286" label="就诊医院编码" />
					<powersi:textfield id="bke051" name="dto.bke051"  readonly="true" label="入院诊断" />
					<powersi:hidden id="bke046" name="dto.bke046" label="入院诊断编码" />
					<powersi:textfield id="ake014" name="dto.ake014" readonly="true" key="ake014" mask="date" label="入院日期" />
				</tr>
			</powersi:editorlayout>
		</powersi:groupbox>
		<!-- 用于初始化设置不可用的div --><div id="divshield"><powersi:groupbox title="基本信息">
			<powersi:editorlayout cols="8">
				<tr>
					...
					<powersi:textfield id="bke011" required="true" name="dto.bke011" colspan="3" maxlength="200" label="申请原因" />
				...
				<powersi:textfield id="bke033" name="dto.bke033" key="bke033" validate="funcCall[checkFunc]" label="联系电话" />
					<powersi:textfield id="bke001" name="dto.bke001" colspan="3" maxlength="80" label="详细地址" />
					<powersi:textfield id="aae011" name="dto.aae011" key="aae011" value="<%=userName %>" disabled="true" label="经办人"/>
				</tr>
			</powersi:editorlayout>
		</powersi:groupbox>
...
<script type="text/javascript">
	...
	function initForm() {
		...
		$("#bke020").val(getdateString());
		$("#bke019").val(getdateString());
	}
	...
	*/
	var tabKeyConf=["akb021:1","aae030:1","aae031:1","bke033","bke001"];
	//配置特殊的事件
	function configSpecialKey(keyid){
		...
		if ("aka121" == keyid) {//选择申请疾病
			chooseDis('',$("#aka120")[0], $("#aka121")[0]);
			val++;
		}
		if ("bkb023" == keyid) {//选择专家疾病
			chooseDis('',$("#bke025")[0], $("#bkb023")[0]);
			val++;
		}
		return val;
	}
	...
	function findAac002(argName,argValue) {
		...
		postJSON("${path}!getPersonInfo.action", param, function(json){
	    	...
	    	if (json.data != "no") {
				...
				/* 查询出院业务窗口 */
			    popupDialog(rootPath +"/medicarespecial/specialManager!openBiz.action?dto.aac001="
			    		+$("#aac001").val()+"&&dto.pageFlag="+$("#pageFlag").val(),500, 800,
			    		function(ret){
					    	if (ret != null) {
					    		$("#aaz217").val(ret.aaz217);
					    		$("#bke053").val(ret.akb021);
					    		$("#aaz286").val(ret.akb020);
					    		$("#bke051").val(ret.bkz101);
					    		$("#bke046").val(ret.akc193);
					    		$("#ake014").val(ret.aae030);
					    		changeShield(0);
								//全键盘操作
								firstTabFocus();
					    	}else{
					    		popupAlert("请选择住院业务信息！");
					    	}	
			    		});
			}
	    });
