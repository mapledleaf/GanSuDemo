...
<powersi:html>
<head>
<powersi:head title="家庭病床就医回退" />
<script type="text/javascript" src="${rootPath}/resource/js/clinicUtils.js"></script>
</head>
<body>
	<powersi:form id="queryInHospitalHomeSickbed" namespace="/inhospital"
		action="InhospitalManagerAction!queryInHospitalHomeSickbed.action">
	...
	<powersi:panelbox key="业务列表">
		<powersi:datagrid id="grid" formId="queryInHospitalHomeSickbed"
	...
	<script type="text/javascript">
		...
		function doQuery(){
			...
			$("#queryInHospitalHomeSickbed").submit();
		}
		...
		function renderOperate(row, index, value) {
			var a = [];
			a.push('<input type="button" value="取消家庭病床登记" class="linkButton"');
...
// 			a.push('<input type="button" value="取消家庭病床出院" class="linkButton"');
...
// 			a.push(" />");
			...
			a.push('<input type="button" value="取消家庭病床结算" class="linkButton"');
		...
		function doCancelOutCharge(index) {
			...
			if(popupConfirm("您确认取消" + aac003 + "的家庭病床出院结算吗?", "确认", function(isOk){
				if(isOk){
					postJSON(
		...
		function doCancelOutRegister(index) {
			...
			if (!confirm("您确认取消" + aac003 + "的家庭病床出院登记吗?")) {
				return;
			}
			postJSON(
