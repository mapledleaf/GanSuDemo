...
%>
<%//TS19112600047   【需求开发】离休人员刷卡提示消费记录（结算云） 医保二部--赵银溪 2019/12/01%>
<powersi:html>
<head>
<powersi:head title="离休门诊收费" />
	...
	<powersi:form id="bizForm" namespace="/diagnose"
		...
		<powersi:panelbox key="查询条件" allowCollapse="false">
			<powersi:panelbox-toolbar>
				...
				<powersi:button id="btnCharge"  key="30天消费查询" onclick="chargeListQuery()" disabled="false" />
		...
		<tags:mediBizInfo_diagnose_charge_retire />
	    <tags:mediFeeInfo_datagrid_retire />
	    <tags:mediHisPayInfo />
...
<script type="text/javascript">
					...
					function(json) {
						...
						if (json.errortype == '0') {
							if("340" != json.data.personinfo.aae140){
								popupAlert("非离休人员不能办理离休门诊业务！");
								$("#querystring").attr("disabled", false);
								init();
								return;
							}
							$.each(json.data.personinfo, function(key, value) {
	...
	function chargeListQuery(){
		var aac001 = powersi.trim($("#aac001").val());
		popupDialog(
				{
					url :"${rootPath}/diagnose/GetPersonInfoAction!queryChargeretire.action?diagnoseInfoDTO.aac001="
					+ aac001,
					onClosed : function() {
