	...
	<powersi:form id="mainForm" action="specialManager!getSpeAppInfList.action" namespace="/medicarespecial">
		<powersi:panelbox title="查询条件">
			<powersi:panelbox-toolbar>
					...
					<powersi:button key="button_insert" value="新 增" data-hotkey='alt+A' onclick="getAddPage();"/>				
			</powersi:panelbox-toolbar>
			...
			<div style="display: none">
				...
				<powersi:textfield id="audit_status" name="dto.audit_status" key="audit_status" value="0" label="审核状态"/>
			...
			<powersi:editorlayout cols="8">
				<tr>
					...
					<powersi:hidden id="aac001" name="dto.aac001"
 					...
 					<powersi:hidden id="aac002" name="dto.aac002"
...
<script type="text/javascript">
	...
	function renderOperate1(row, index, value) {
		try {
			...
			a.push('<input title="单击进入申请修改页面" type="button" value="修改" class="linkButton"');
			a.push(' onclick="getModifyPage(');
			a.push('\'' + row.aaz267 + '\'');
			a.push(',\'' + row.aka083 + '\'');
			a.push(')"');
			a.push(" />");
			
			a.push(' <input title="单击可删除申请" type="button" value="删除" class="linkButton"');
	...
	//新增申请页面
	function getAddPage() {
		var param = "dto.aae140=" + $("#aae140").val() + "&dto.pageFlag=13"  + "&dto.aka083=" + $("#aka083").val()
		+ "&dto.aaa027=" + $("#aaa027").val() + "&dto.caa027=" +$("#caa027").val() + "&dto.auditType=" + $("#auditType").val();
		popupDialogWithParam({
			url: "${path}!getApplyAddPage.action?" + param,
			onClosed: function(){
				//保存成功
				if(this.returnValue) {
					queryAll();
		...
		}, null, getPageInitParamByApplyType('131', 'modifyPH'),pwidth);
	}
	...
	function getModifyPage(aaz267, aka083) {
		var param = "dto.aae140=" + $("#aae140").val() + "&dto.pageFlag=" + getPageInitParamByApplyType(aka083, 'pageFlag') + "&dto.aaz267=" + aaz267 
		+ "&dto.aaa027=" + $("#aaa027").val() + "&dto.caa027=" +$("#caa027").val() + "&dto.auditType=" + $("#auditType").val()+ "&dto.modify_131=13";
		popupDialogWithParam({
