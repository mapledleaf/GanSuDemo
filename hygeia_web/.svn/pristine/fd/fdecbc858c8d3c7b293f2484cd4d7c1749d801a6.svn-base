...
layui.define('view', function (exports) {
    var $ = layui.jquery
        ...
        , admin = {
            ...
            , tabsBodyChange: function (index, options) {
                ...
                admin.tabsBody(index).addClass(SHOW);
                ...
                try{
                	...
                	if(iframe && iframe.length > 0){
                		...
                		if(iframeWin && $.isFunction(iframeWin.refreshPageSize)) {
                			iframeWin.refreshPageSize(refreshPageSize);
                		}
                	}
                } catch(ex){
                	//alert(ex.message);
            ...
            //关闭选择标签页
            , closeSelectTabs: function (id) {
            	var TABS_REMOVE = 'LAY-system-pagetabs-remove';
            	$(TABS_HEADER).each(function (index, item) {
            		var othis = $(item);
                    if (othis.attr('lay-id') == id || othis.attr('lay-attr') == id) {
                        othis.addClass(TABS_REMOVE);
                        admin.tabsBody(index).addClass(TABS_REMOVE);
                    }
                });
                var tabids = [];
                $('.' + TABS_REMOVE).each(function(){
                    var othis = $(this);
                    tabids.push(othis.find(".layadmin-iframe").data('tabid'));
                    othis.remove();
                });
                layui.index.removeTabsPage(tabids);
            }
            //……
        };
    ...
    var events = admin.events = {
        ...
        , refresh: function () {
            ...
            //显示加载图标
            $(APP_BODY).find('.layadmin-tabs-loading[data-tabid="' + iframe.data("tabid") + '"]').show();
        ...
        , fullscreen: function (othis) {
            ...
            if (iconElem.hasClass(SCREEN_FULL)) {
                //var elem = document.body;//使用body打开iframe会黑屏
        ...
        , closeOtherTabs: function (type) {
            ...
            if (type === 'all') {
                ...
                var tabids = [];
                $(APP_BODY).find('.' + TABS_BODY + ':gt(0)').each(function(){
                    var othis = $(this);
                    tabids.push(othis.find(".layadmin-iframe").data('tabid'));
                    othis.remove();
                });
    ...
    !function () {
        ...
        if (elem.webkitRequestFullScreen || elem.mozRequestFullScreen || elem.requestFullScreen || elem.msRequestFullscreen) {
            
        } else {
        	$('a[layadmin-event="fullscreen"]').parent().hide();
        }
    }();
    ...
    element.on('tabDelete(' + FILTER_TAB_TBAS + ')', function (obj) {
        ...
        if(obj.index){
            layui.index.removeTabsPage(admin.tabsBody(obj.index).find(".layadmin-iframe").data('tabid'));
            admin.tabsBody(obj.index).remove();
        }
        ...
        //fix
        admin.tabsPage.index = $(TABS_HEADER + '.layui-this').index();
    ...
    $body.on('click', '*[lay-href]', function () {
        ...
        topLayui.index.openTabsPage(href, text || othis.text(), id);
    });
    ...
    //窗口resize事件
    $('#LAY_app_favorite_list').css('max-height', $(window).height() - 70);
