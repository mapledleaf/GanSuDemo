...
import com.powersi.sys.manager.dao.BeanDAO;
...
/**
 * The Class BeanManagerAction.
 */
@Action("BeanManager")
public class BeanManagerAction extends UploadAction {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 1L;

	/** The Constant CACHE_NAME. */
	private static final String CACHE_NAME = "bean";
	
	/** The dao. */
	private static BeanDAO dao = BeanHelper.getBean(BeanDAO.class);
	...
	public String upload() {
		try {
			...
			if (!UtilFunc.startsWithIgnoreCase(uploadsFileName.get(0),
					"sys_beans")) {
				saveJSONError("导入文件名必须使用sys_beans开始");
				return NONE;
			}
