...
<body>
	...
	<powersi:form id="outCharge" namespace="/inhospital"
		...
		<powersi:hidden id="comm_subsys_flag" name="inHospitalDTO.comm_subsys_flag" value="1"/>
	</powersi:form>
	...
	<script type="text/javascript">
		...
		function findOutChargeaaz217() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findOutChargeaaz217.action?inHospitalDTO.querystring="
		...
		function billContent() {
			...
			var bae410 = "02";
			postJSON(
					"${rootPath}/comminter/InvoiceManagerAction!queryKab1.action",{"kab1DTO.bae410":"02"}, function(json) {
						if (!checkJSONResult(json)) {
							return;
						}
						if(json.data.kab1DTO.length <= 0){
							popupInfo("住院未领用发票无法打印发票");
						}else{
							popupDialog(
									{
										url : "${rootPath}/comminter/InvoiceManagerAction!updateAndGetKab1.action?kab1DTO.bae410="+bae410+"&flag=1&kab1DTO.aaz217="+aaz217,
										onClosed : function() {
										}
									}, 600, 1000);
						}
