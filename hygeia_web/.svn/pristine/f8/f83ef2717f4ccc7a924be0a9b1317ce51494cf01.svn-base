...
<body>
	<powersi:form id="inputFeeSpecial" namespace="/inhospital"
		action="InhospitalManagerAction!inputFeeSpecial.action">
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findaaz217Special()" buttonText="读卡"
	...
	<powersi:panelbox cssStyle="display:none;">
		<powersi:form id="feeformSpecial" namespace="/inhospital"
			action="InhospitalManagerAction!feeSelectSpecial.action">
	...
	<powersi:panelbox key="费用明细">
		...
		<powersi:datagrid id="grid" formId="feeformSpecial" delayLoad="true"
	...
	<script type="text/javascript">
		...
		function readic() {
			...
			findaaz217SpecialByBka100();
		}

		function findaaz217SpecialByBka100() {
			var inputFeeData = $("#inputFeeSpecial").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findaaz217Special.action",
		...
		function findaaz217Special() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findaaz217Special.action?inHospitalDTO.querystring="
		...
		function saveFee() {
			...
			var saveItemData = $("#inputFeeSpecial").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!inputFeeSpecial.action",
		...
		function tryCalcTreatSpecial() {
			...
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!tryCalcTreatSpecial.action?inHospitalDTO.kcd1id="
