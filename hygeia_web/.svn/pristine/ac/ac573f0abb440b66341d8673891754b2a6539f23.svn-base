...
<powersi:html>
<head>
<powersi:head title="修改疾病目录" target="_self" />
</head>
<body>
	<powersi:form id="mainform" disabled="true">
		<powersi:editorlayout cols="8">
			...
			<powersi:editorlayout-row>
				<powersi:textfield id="aka420" name="diseaseDto.aka420" label="医院疾病编码" required="true" readonly="true"/>
			...
			<powersi:editorlayout-row>
				...
				<powersi:hidden id="ka60id" name="diseaseDto.ka60id"/>
			</powersi:editorlayout-row>
...
<script type="text/javascript">
	...
	function save() {
		...
		postJSON("${rootPath}/medicare/HospCataAction!updateHospDisease.action",saveItemData, function(json) {
   				if (!checkJSONResult(json)) {
   					return;
   				}
   				popupInfo(json.data);
   				if(json.data == '修改目录信息成功！'){
   					closeDialog();
   				}
   			});
	}
