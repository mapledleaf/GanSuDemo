...
<powersi:html>
<head>
<powersi:head title="异地就诊信息查询" />
</head>
<body>
	<powersi:form id="queryForm" namespace="/query"
		action="BizQueryAction!queryAllBusinessInfoYdjy.action">
...
<%-- 				<powersi:button id="" label="基金查询" onclick="fundStatusInfo()" /> --%>
			...
			<powersi:editorlayout cols="10%,15%,8%,12%,8%,12%,9%,7%,13%">
				<tr>
					<powersi:codeselect id="arg_name" label="人员信息"
							name="bizQueryDto.arg_name"
					...
					<td>
						<select id="zys001" name="bizQueryDto.zys001" class="" onchange="changButDis()">
							<option value="1">在院</option>
							<option value="2">出院</option>
						</select>
					</td>
					<td align="right">
						<input type="radio" id="clinicDate" name="radio_date" value="rad_fromdate" checked="checked" onchange="changeDatetype()" />
						<label id="inhospID">入院时间</label>
					</td>
				...
				<tr>
					...
					<td>
						<select id="yd_flag" name="bizQueryDto.yd_flag" >
							<option value="0">全部</option>
							<option value="2">省内异地</option>
							<option value="4">跨省异地</option>
						</select>
					</td>
					<td colspan="2"></td>
					<td align="right" class="tdLabel">结算状态</td>
					<td>
						<select id="zys002" name="bizQueryDto.zys002"
					...
					<td align="right">
						<input type="radio" id="endclinicDate" name="radio_date" value="rad_todate" onchange="changeDatetype()"/>
						<label id="settleDate">结算时间</label>
					</td>
				...
				<powersi:editorlayout-row>
					...
					<powersi:hidden id="one" name="one" value="0"/>
					...
					<powersi:hidden id="center_flag" name="bizQueryDto.center_flag" value="1" />
				</powersi:editorlayout-row>
			...
			<div id="yyDiv">
				<powersi:panelbox key="panel_result" title="业务信息"
					...
					<powersi:datagrid id="query_zy_bizGridId" formId="queryForm"
						...
						<powersi:datagrid-column label="人员类别" id="bka035" name="bka035" width="80" />
						...
						<powersi:datagrid-column label="入院时间" id="aae030" name="aae030"
							format="{0,date,yyyy-MM-dd}" />
						<powersi:datagrid-column label="入院诊断" id="bkz101" name="bkz101" />
						<powersi:datagrid-column label="出院时间" id="aae031" name="aae031" 
							 format="{0,date,yyyy-MM-dd}" />
						<powersi:datagrid-column label="出院诊断" id="bkz101" name="bkz101" />
						<powersi:datagrid-column label="住院号" id="akc190" name="akc190" />
						<powersi:datagrid-column label="病区" id="bka022" name="bka022" width="80" />
						<powersi:datagrid-column label="科室" id="bka020" name="bka020" width="80" />
						<powersi:datagrid-column label="床位号" id="ake020" name="ake020" width="80" />
						<powersi:datagrid-column label="单位名称" id="bka008" name="bka008" />
						<powersi:datagrid-column label="结算时间" id="aae031" name="aae031" 
							 format="{0,date,yyyy-MM-dd}" />
						<powersi:datagrid-column label="公务员级别" id="bac001" name="bac001"
							width="80" code="bac001" hide="true" />
						<powersi:datagrid-column label="辖区" id="aaa027" name="aaa027" width="80" code="aaa027" hide="true" />
						<powersi:datagrid-column label="业务交接号" id="bae010" name="bae010" hide="true" />
						<powersi:datagrid-column label="报账类型" id="bka037" name="bka037"
							code="bka037List" render="renderDictionary" hide="true" />
		...
		<div class="col-3">
			<powersi:panelbox iconClass="icon-cog" title="费用信息"
				...
				<powersi:datagrid id="feeGridId" delayLoad="true" showReload="false"
					...
					<powersi:datagrid-column id="aka063" name="aka063"
						key="aka063" label="费用类别" code="aka063List"
	...
	<script type="text/javascript">
		...
		function onSelectClick(index) {
			...
			feeGridId.reset();
			postJSON(
					"${rootPath}/query/BizQueryAction!queryBizFeeInfoYdjy.action?bizQueryDto.aaz217="
							+ aaz217,
					afterInHospBizFee);
		}
		...
		function feeDetailTable() {
			...
			var fromdate = row.aae030;
			var secfalg = row.secfalg;
			var one = row.one;
			popupDialog(
					{
						url : "${rootPath}/pages/biz/medicare/query/InhospitalBizQueryFeeDetailYdjy.jsp?bizQueryDto.akb020="
								+ akb020
								+ "&bizQueryDto.aaz217="
								+ aaz217
								+ "&bizQueryDto.secfalg="
			...
			popupDialog(
					{
						url : "${rootPath}/common/SettlementReportAction!settlementReportYdjy.action"
								+"?bizQueryDTO.aae030=" + row.aae030
								+"&bizQueryDTO.aae031=" + row.aae031
