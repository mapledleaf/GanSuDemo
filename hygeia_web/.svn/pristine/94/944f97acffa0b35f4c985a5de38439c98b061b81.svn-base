 ...
 * 服务类生成器.
 */
public abstract class ServiceCodeGen {
	...
	public static void main(String[] args) {
		try {
			...
			String name = "";
			String comment = "";
			String packageName = "com.powersi.temp.service";
			...
			while (UtilFunc.isEmpty(name)) {
				line = ConsoleHelper.readLine("请输入服务名(不需要添加 Service，注意大小写)：");
				if (UtilFunc.hasText(line)) {
					name = line.trim();
				}
			}
			sb.append("服务名：").append(name).append(UtilFunc.NEW_LINE);

			while (UtilFunc.isEmpty(comment)) {
				line = ConsoleHelper.readLine("请输入服务注释：");
				if (UtilFunc.hasText(line)) {
					comment = line.trim();
				}
			}
			sb.append("服务注释：").append(comment).append(UtilFunc.NEW_LINE);
