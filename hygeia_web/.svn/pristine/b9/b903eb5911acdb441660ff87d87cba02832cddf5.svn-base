...
import com.powersi.sys.manager.dao.TextDAO;
...
/**
 * The Class TextManagerAction.
 */
@Action("TextManager")
public class TextManagerAction extends UploadAction {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = 1L;

	/** The Constant CACHE_NAME. */
	private static final String CACHE_NAME = "systext";
	
	/** The dao. */
	private static TextDAO dao = BeanHelper.getBean(TextDAO.class);
	...
	public String upload() {
		try {
			...
			if (!UtilFunc.startsWithIgnoreCase(uploadsFileName.get(0),
					"sys_texts")) {
				saveJSONError("导入文件名必须使用sys_texts开始");
				return NONE;
			}
