...
<body>
	<powersi:form id="inputFeeSnyd" namespace="/inhospital"
		action="InhospitalManagerAction!inputFeeSnyd.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btSaveFee" label="保存费用" onclick="saveFeeSnyd()" />
				<powersi:button id="btSelectFee" label="查询费用"
					onclick="selectFeeSnyd()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findaaz217Snyd()" buttonText="读卡"
	...
	<powersi:panelbox cssStyle="display:none;">
		<powersi:form id="feeformSnyd" namespace="/inhospital"
			action="InhospitalManagerAction!feeSelectSnyd.action">
	...
	<div id="divTab2" class="row">
		<div class="col-8">
			<powersi:panelbox key="费用明细">
				...
				<powersi:editorlayout
					cols="6%,9%,1%,10%,5%,8%,6%,8%,6%,8%,6%,8%,6%,13%">
					<powersi:editorlayout-row>
				...
				<powersi:datagrid id="grid" formId="feeformSnyd" delayLoad="true"
					showReload="false" autoWidth="true" enabledSort="false"
					alternatingRow="true" checkbox="true" colDraggable="false" height="300"
					totalRender="renderTotal" pageSize="30">
					 <powersi:datagrid-column name="bkm017" display="药监本位码" width="1%" 
				   		hide="true"/>
				   	<powersi:datagrid-column name="bka073" display="限制用药标志" width="1%"
						hide="true" />
	...
	<script type="text/javascript">
		...
		function readic() {
			...
			findaaz217SnydByBka100();
		}

		function findaaz217SnydByBka100() {
			var inputFeeData = $("#inputFeeSnyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findaaz217Snyd.action",
		...
		function findaaz217Snyd() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findaaz217Snyd.action?inHospitalDTO.querystring="
		...
		function saveFeeSnyd() {
			...
			var saveItemData = $("#inputFeeSnyd").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!inputFeeSnyd.action",
		...
		function tryCalcTreatSnyd() {
			...
			feeItemReset();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!tryCalcTreatSnyd.action?inHospitalDTO.kcd1id="
		...
		function insertfee() {
			...
			var bkm017 = jsonFee.bkm017;
			postJSON(
					"${rootPath}/common/CommonManagerAction!queryMemoryDB.action",
					{
						"bkm017" : bkm017
					},
					function(json) {
						...
						if (suss == 1) {
							var ake006 = json.data.ake006;
							var bkc102 = json.data.bkc102;
							if(bkc102==null || bkc102 == ""){
								jsonFee.bka073 = "1";
								grid.add(jsonFee);	
								return;
							}
							popupConfirm('【' + ake006 + '】，该药品为（' + bkc102+ '）使用药品！是【可报销】，否【自费】？ ', '提示:医保限制用药',function(yes) {
								//更改先自付比例
								if (yes) {
									jsonFee.bka073 = "1";
									grid.add(jsonFee);	
