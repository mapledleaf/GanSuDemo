...
<body>
	<powersi:form id="outRegisterHomeSickbed" namespace="/inhospital"
		action="InhospitalManagerAction!outRegisterHomeSickbed.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btTryCalcTreat" label="计算费用"
					onclick="outRegisterTryCalcTreatHomeSickbedBiz()" />
				<powersi:button id="btSaveOutRegister" label="保存登记"
					onclick="saveOutRegisterHomeSickbed()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findOutRegisteraaz217HomeSickbed()" buttonText="读卡"
	...
	<script type="text/javascript">
		...
		function readic() {
			...
			findOutRegisteraaz217HomeSickbedByBka100();
		}

		function findOutRegisteraaz217HomeSickbedByBka100() {
			var outRegisterData = $("#outRegisterHomeSickbed").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findOutRegisteraaz217HomeSickbed.action",
		...
		/*查询在院登记业务信息*/
		function findOutRegisteraaz217HomeSickbed() {
			if (window.event.keyCode == 13) {
				$("#bke548").val("");
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findOutRegisteraaz217HomeSickbed.action?inHospitalDTO.querystring="
		...
		/*费用计算(试算)*/
		function outRegisterTryCalcTreatHomeSickbedBiz() {
			if (!checkForm()) {
				return;
			}
			...
			feeItemReset();
			var dataOutRegister = $("#outRegisterHomeSickbed").serialize();
			$("#btTryCalcTreat").prop("disabled", true);
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterTryCalcTreatHomeSickbed.action",
		...
		/*保存出院*/
		function saveOutRegisterHomeSickbed() {
			if (!checkForm()) {
				return;
			}
			var aac003 = powersi.trim($("#aac003").val());
			if (!confirm("您确认保存[" + aac003 + "]的家庭病床出院登记吗?")) {
				return;
			}
			$("#btSaveOutRegister").prop("disabled", true);
			var dataOutRegister = $("#outRegisterHomeSickbed").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!outRegisterHomeSickbed.action",
					dataOutRegister, function(json) {
						$("#btSaveOutRegister").prop("disabled", false);
						if (!checkJSONResult(json)) {
							return;
