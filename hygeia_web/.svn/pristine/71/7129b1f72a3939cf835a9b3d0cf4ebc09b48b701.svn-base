...
<script type="text/javascript" src="${rootPath}/pages/biz/medicare/inhospital/InputFee.js?V201706161"></script>
<style>
.showpay {
...
<body>
	<powersi:form id="inputFeeMaternity" namespace="/inhospital"
		action="InhospitalManagerAction!inputFeeMaternity.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				<powersi:button id="btSelectFee" label="查询费用" onclick="selectFee()" key="button_query"/>
				<powersi:button id="btSaveFee" label="保存费用" key="button_save_register"
					onclick="saveFeeMaternity()" />
				<powersi:button id="btTryCalcTreat" label="试算费用" key="button_calc0"
					onclick="tryCalcTreatMaternity()" />
			...
			<powersi:editorlayout cols="10%,10%,15%,8%,12%,8%,12%,10%,15%">
				<powersi:editorlayout-row>
					...
					<td>
						...
						placeholder="请输入信息回车！" readonly="false" onkeyup="findAaz217Maternity()" buttonDisabled="false" />
					</td>
		...
		<powersi:panelbox key="业务信息">
			<powersi:editorlayout cols="8">
				...
				<powersi:editorlayout-row>
					...
					<powersi:textfield id="bkz101" name="inHospitalDTO.bkz101" label="入院诊断" readonly="true" codeType="bke414"/>
	...
	<powersi:panelbox cssStyle="display:none;">
		<powersi:form id="feeformMaternity" namespace="/inhospital"
			action="InhospitalManagerAction!feeSelectMaternity.action">
...
<%-- 			</powersi:form> --%>
<%-- 		</powersi:panelbox> --%>
		<powersi:datagrid id="grid" formId="feeformMaternity" delayLoad="true"
			...
			<powersi:datagrid-column name="bkc194" display="匹配ID" width="1%" hide="true" />
			<powersi:datagrid-column name="bkc109" display="异名ID" width="1%" hide="true" />
			<powersi:datagrid-column name="aka065" code="aka065" display="等级" width="6%" />
			<powersi:datagrid-column name="ake003" code="ake003" display="类别" width="6%" />
			...
			<powersi:datagrid-column name="bka052" code="bka052" display="剂型" width="6%" />
	...
	<script type="text/javascript">
		var actionUrl = "${rootPath}/common/CommonManagerAction!chooseHospitalCatalogMatched.action?searchType=py&ake007="
			+ $("#ake007").val()+"&inHospitalDTO.caa027="+$("#caa027").val()+"&type=2";//2套餐
		...
		function findaaz217MaternityByBka100() {
			var inputFeeData = $("#inputFeeMaternity").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findAaz217Maternity.action",
		...
		function findAaz217Maternity() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findAaz217Maternity.action",
								{"inHospitalDTO.querystring" : querystring, "inHospitalDTO.argName" : argName}, 
								function(json) {
									if (!checkJSONResultNew(json)) {
										return;
									}
									...
									$("#btSaveFee").prop("disabled", false);
									if ("521" == json.data.bka006) {
										$("#akb066").prop("disabled", false);
									} else {
										$("#akb066").prop("disabled", true);
									}
								});
			}
		}
		...
		function saveFeeMaternity() {
			...
			var saveItemData = $("#inputFeeMaternity").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!inputFeeMaternity.action",
		...
		/*费用计算(试算)*/
		function tryCalcTreatMaternity() {
			$("#aae140").val("510");
			...
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!tryCalcTreatMaternity.action?inHospitalDTO.kcd1id="
							+ kcd1id + "&inHospitalDTO.aaz217=" + aaz217
							+ "&inHospitalDTO.aae240=" + akb066,
					function(json) {
						$("#btTryCalcTreat").prop("disabled", false);
						if (!checkJSONResultNew(json)) {
							return;
		...
		function dealKeyDown(para) {
			if (window.event.keyCode == "13") {
				...
				if ("todayCollect" == fieldName) {
					...
					$("#needFind").val(
							(parseFloat(todayCollect) - parseFloat(bka831))
									.toFixed(2));
				}
			}
		}
		...
		function deletefee() {
			...
			popupConfirm("是否删除已选择的费用?", "确认", function(isOk){
				if(isOk){
