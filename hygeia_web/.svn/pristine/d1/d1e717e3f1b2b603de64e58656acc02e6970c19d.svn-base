...
<body>
	...
	<powersi:form id="bizForm" namespace="/diagnose"
		...
		<powersi:panelbox key="查询条件" allowCollapse="false">
			...
			<powersi:editorlayout cols="7%,12%,5%,8%,8%,17%,7%,13%,5%,13%">
		...
		<tags:mediBizInfo_diagnose_charge_bigdisease />
   		<tags:mediFeeInfo_datagrid_bigdisease />
    	<tags:mediHisPayInfo />
			...
			postJSON(
					...
					function(json) {
						...
						if (json.errortype == '0') {
							...
							if(!powersi.isnull(json.data.spinfo)){
								var spinfo = json.data.spinfo[0];
								$("#aaz267").val(spinfo.aaz267);
								$("#bkm030").val(spinfo.aae030);//开始时间
								$("#bkm031").val(spinfo.aae031);//结束时间
								$("#bkz101").val(spinfo.aka121);//疾病编码
								$("#akc193").val(spinfo.aka120);//疾病诊断
								$("#ake001").val(spinfo.ake001);//申请药品
								$("#ake002").val(spinfo.ake002);//申请药品
								$("#akb081").val(spinfo.akb081);//申请限额
								$("#bkb017").val(spinfo.bkb017);//剩余限额
								loadBigGrid();
							}
			...
			url : "${rootPath}/diagnose/GetPersonInfoAction!getPersonBusi.action?diagnoseInfoDTO.arg_name="
					...
					onClosed : function() {
						...
						if (ret != null) {
							...
							postJSON("${rootPath}/medicare/GetPersonInfoAction!getPersonBusiDetail.action?akb020="
									...
									function(json) {
										...
										//TS19051500140   大病特药退费报错,收费和退费先支付不一致导致   重新从申请中获取申请药品编码写入缓存   cj  20190517
										$("#ake001").val(speinfo.ake001);
										$("#aaz267").val(speinfo.aaz267);
										$("#bkm030").val(speinfo.aae030);//开始时间
										$("#bkm031").val(speinfo.aae031);//结束时间
										$("#ake002").val(speinfo.ake002);//申请药品
										$("#akb081").val(speinfo.akb081);//申请限额
										$("#bkb017").val(speinfo.bkb017);//剩余限额
										loadBigGrid();
