...
<powersi:html>
<head>
<powersi:head title="医院自查申诉处理" target="_self"/>
<script src="${strutsPath}/include/fileupload.js" type="text/javascript"></script>

</head>
<body>
<powersi:form name="mainForm" id="mainForm"  >
	<powersi:hidden id="aaz010" name="#request.hospitalMap.aaz010"/>
	<powersi:hidden id="aaz328" name="#request.hospitalMap.aaz328"/>
	<div class="col-12">
		<powersi:panelbox title="基本信息" iconClass="icon-inbox" allowCollapse="false">
			<powersi:editorlayout cols="8">
				<powersi:editorlayout-row>
					<powersi:textfield name="#request.hospitalMap.akb021" label="医院名称" readonly="true" />
					<powersi:textfield name="#request.hospitalMap.akb020" label="医院编号" readonly="true" />
					<powersi:textfield name="#request.hospitalMap.akb022_name" label="医院类型" readonly="true" />
					<powersi:textfield name="#request.hospitalMap.aka101_name" label="医院等级" readonly="true" />
				</powersi:editorlayout-row>
				<powersi:editorlayout-row>
					<powersi:textfield name="#request.hospitalMap.aae004" label="医保联系人" readonly="true" />
					<powersi:textfield name="#request.hospitalMap.aae006" label="地址" readonly="true" colspan="5"/>
				</powersi:editorlayout-row>
...
function showDoubt(aaz328){
	var url = "${rootPath}/medicare/DeclarePayAction!getStayAppealDetail.action";
	var params = {};
...
function initData4Complaint(){
	var aaz328 = $("#aaz328").val();
	var url = "${rootPath}/medicare/DeclarePayAction!getAppealDataInfo.action?bpd413="+aaz328;
	gridComplaint.set("url",url);
}

function initAppealExplain(aaz328){
	var url = "${rootPath}/medicare/DeclarePayAction!getAppealExplainList.action";
	var params = {};
	params.aaz328 = aaz328;
