...
<body>
	<powersi:form id="bizForm" namespace="/diagnose"
		...
		<tags:mediBizInfo_birth_charge_remote />
...
<script type="text/javascript">
	...
	function calc(str) {
		...
		//NTS20072200316 湘潭市异地门特业务收费时优化 异地生育门诊通过身份证不需要采集照片，此功能先暂时屏蔽 陈洁  20200722
		<%-- if(<%=isPic%>){
			...
			postJSON(
					...
					function(json) {
						...
						if (json.errortype == '0') {
							...
							if (json.data.injuryorbirthinfo != null){
								var injuryorbirthinfo = json.data.injuryorbirthinfo[0];
								$("#serial_apply").val(injuryorbirthinfo.serial_bo_no);
								$("#identify_code").val(injuryorbirthinfo.identify_code);
								$("#certificate_no").val(injuryorbirthinfo.certificate_no);
								$("#begin_date").val(injuryorbirthinfo.begin_date);
								$("#end_date").val(injuryorbirthinfo.end_date);
								$("#birth_type").val(injuryorbirthinfo.birth_type);
								$("#birth_type_name").val(injuryorbirthinfo.birth_type_name);
								$("#childbirth_no").val(injuryorbirthinfo.childbirth_no);
							}
						} 
						...
						if (ret != null) {
							...
							postJSON(
									...
									function(json) {
										if (!checkJSONResult(json)) 
											return;
										$("#idcard").val(ret.idcard.trim());
										$("#name").val(ret.name.trim());
										$("#corp_name").val(ret.corp_name.trim());
										$("#pers_type").val(ret.pers_type.trim());
										$("#sex").val(ret.sex.trim());
										$("#birthday").val(ret.birthday.trim());

										$("#indi_id").val(ret.indi_id.trim());
										
										$("#treatment_type").val(ret.treatment_type.trim());
										$("#treatment_type_reg").val(ret.treatment_type.trim());
										$("#telephone").val(ret.telephone.trim());
										$("#serial_no").val(ret.serial_no.trim());
										$("#serial_apply").val(ret.serial_apply.trim());
										$("#diagnose").val(ret.icd.trim());
                                        $("#in_disease_name").val(ret.disease.trim());
                                        $("#diagnose_date").val(ret.diagnose_date.trim());
