	...
	<powersi:form id="bizForm" namespace="/diagnose"
		...
		<powersi:panelbox key="查询条件" allowCollapse="false">
			<powersi:panelbox-toolbar>
				...
				<powersi:button id="btnFinger" key="指静脉比对" onclick="fingerVeins()" disabled="false" />
		...
		<tags:mediBizInfo_special_charge />
  		<tags:mediFeeInfo_datagrid_special />
    	<tags:mediHisPayInfo />
		<powersi:hidden id="reduceflag" key="退改费类型"
			name="diagnoseInfoDTO.reduceflag" value="0" />
		<powersi:hidden id="bka975" name="inHospitalDTO.bka975" />
					...
					function(json) {
						...
						if (json.errortype == '0') {
							...
							if (json.data.spinfo != null){
								...
								$("#akc193").val(spinfo.akc193);
								$("#bkz101").val(spinfo.bkz101);
								$("#bkm030").val(spinfo.aae030);
								$("#bkm031").val(spinfo.aae031);
								$("#akb081").val(spinfo.akb081);
								$("#S1310").val(spinfo.S1310);
								$("#Q1310").val(spinfo.Q1310);
								$("#C1310").val(spinfo.C1310);
								$("#ky_money").val(spinfo.ky_money);
								$("#this_money").val(spinfo.this_money);
								$("#inhosp_money").val(spinfo.inhosp_money);
							}
							if (json.data.hospinfo != null){
								var hospinfo = json.data.hospinfo[0];
								$("#bka975").val(hospinfo.bka975);
							}
						...
						if (ret != null) {
							...
							postJSON(
									...
									function(json) {
										...
										$("#akc193").val(speinfo.akc193);
										$("#bkm030").val(speinfo.aae030);
										$("#bkm031").val(speinfo.aae031);
										$("#akb081").val(speinfo.akb081);
										$("#S1310").val(speinfo.S1310);
										$("#Q1310").val(speinfo.Q1310);
										$("#C1310").val(speinfo.C1310);
										$("#ky_money").val(speinfo.ky_money);
										$("#this_money").val(speinfo.this_money);
										$("#inhosp_money").val(speinfo.inhosp_money);
