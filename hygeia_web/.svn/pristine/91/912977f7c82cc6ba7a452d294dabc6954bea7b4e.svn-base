...
<body>
	<powersi:form id="checkReferralTransferform">
		<powersi:groupbox key="panel_result" title="个人信息">
		    <powersi:panelbox-toolbar>
			    <powersi:button id="btCheckPass" label="审批通过" 
			        onclick="checkPass()" />
	...
	<script type="text/javascript">
	    ...
	    function checkPass(){
			...
			$("#btCheckPass").prop("disabled", true);
			postJSON("${rootPath}/particular/ParticularManagerAction!updateReferralTransferInfo.action",
					saveItemData, importCheckPassBackInfo);
	    }
	    
	    //审批通过返回
	    function importCheckPassBackInfo(json) {
			...
			$("#btCheckPass").prop("disabled", false);
			closeDialog();
		}
	</script>
</body>
