...
<body>
	...
	<powersi:form id="bizForm" method="post">
		...
		<tags:mediBizInfo_diagnose_charge_maternity />
		<tags:mediFeeInfo_datagrid_maternity />
		<tags:mediHisPayInfo_maternity />
		...
		<powersi:hidden id="reduceflag"  name="diagnoseInfoDTO.reduceflag" key="退改费类型" value="0"/>
		<powersi:hidden id="bka042" name="diagnoseInfoDTO.bka042" key="生育备案号" />
...
<script type="text/javascript">
	...
	function importCalcInfo(json) {
		...
		$("#st_yydf").val((parseFloat(ld_hosp_pay)).toFixed(2));
					...
					function(json) {
						...
						if (json.errortype == '0') {
							...
							if (json.data.injuryorbirthinfo != null){
								$("#bka042").val(json.data.injuryorbirthinfo.bka042);//生育备案号
							}
							...
							postJSON(
									...
									function(json) {
											...
											if (json.errortype == '0') {
												...
												if (json.data.injuryorbirthinfo != null){
													$("#bka042").val(json.data.injuryorbirthinfo.bka042);
												}
												//【NTS20051200240】湘潭-医保电子凭证支付通知需求 electronicVoucher === null 代表没有使用电子凭证 by 钟声 2020/5/13
												if(electronicVoucher === null && $("#argName").val()=="aac002" && isReadCard=="0"){
													doPhoto();
