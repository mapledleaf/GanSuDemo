 ...
 * 数据访问类生成器.
 */
public abstract class DaoCodeGen {
	...
	public static void main(String[] args) {
		try {
			...
			String name = "";
			String comment = "";
			String packageName = "com.powersi.temp.dao";
			...
			System.out.println("----------------------------------------------");
			System.out.println("\t生成dao");
			System.out.println("----------------------------------------------");
			
			StringBuilder sb = new StringBuilder();
			String line = null;
			while (UtilFunc.isEmpty(name)) {
				line = ConsoleHelper.readLine("请输入数据访问名(不需要添加 Dao，注意大小写)：");
				if (UtilFunc.hasText(line)) {
					name = line.trim();
				}
			}
			sb.append("数据访问名：").append(name).append(UtilFunc.NEW_LINE);

			while (UtilFunc.isEmpty(comment)) {
				line = ConsoleHelper.readLine("请输入数据访问注释：");
				if (UtilFunc.hasText(line)) {
					comment = line.trim();
				}
			}
			sb.append("数据访问注释：").append(comment).append(UtilFunc.NEW_LINE);
			...
			DaoCodeGenerator.generateFile(name, comment, packageName,
					srcPath, overWrite, fileEncoding);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
