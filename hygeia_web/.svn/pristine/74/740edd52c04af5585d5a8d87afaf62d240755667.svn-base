...
<body>
	<powersi:form id="inputFeeInjury" namespace="/inhospital"
		action="InhospitalManagerAction!inputFeeInjury.action">
		<powersi:panelbox key="查询条件">
			<powersi:panelbox-toolbar>
				...
				<powersi:button id="btTryCalcTreat" label="试算费用"
					onclick="tryCalcTreatInjury()" />
				<powersi:button id="btSelectFee" label="查询费用"
					onclick="selectFeeInjury()" />
			...
			<powersi:editorlayout cols="8%,17%,8%,17%,8%,17%,8%,17%">
				<powersi:editorlayout-row>
					<powersi:textfield id="querystring"
						...
						onkeydown="findaaz217Injury()" buttonText="读卡"
	...
	<powersi:panelbox cssStyle="display:none;">
		<powersi:form id="feeformInjury" namespace="/inhospital"
			action="InhospitalManagerAction!feeSelectInjury.action">
	...
	<powersi:panelbox key="费用明细">
		...
		<powersi:editorlayout
			...
			<powersi:editorlayout-row>
				...
				<powersi:textfield id="bka056" name="bka056" label="单价" onkeyup="num(this)"
					onkeydown="dealKeyDown(this)" required="true" />
				<powersi:textfield id="bka057" name="bka057" label="数量" onkeyup="num(this)"
					onkeydown="dealKeyDown(this)" required="true" />
				<powersi:textfield id="bka058" name="bka058" label="金额" readonly="true"
					onkeydown="dealKeyDown(this)" required="true" />
				<powersi:textfield id="bka051" name="bka051" label="日期" mask="date"
					onchange="onchangebka051(this)" readonly="false" required="true" />
			</powersi:editorlayout-row>
		</powersi:editorlayout>
		<powersi:datagrid id="grid" formId="feeformInjury" delayLoad="true"
	...
	<script type="text/javascript">
		...
		/* var actionUrl = "${rootPath}/common/CommonManagerAction!chooseHospitalCatalogMatched.action?searchType=py&bka051="
				+ $("#bka051").val(); */
		var actionUrl = "${rootPath}/common/CommonManagerAction!chooseHospitalCatalogMatched.action?searchType=py&bka051="
			+ $("#bka051").val()+"&inHospitalDTO.caa027="+$("#caa027").val()+"&type=2";//2套餐
		...
		function readic() {
			...
			findaaz217InjuryByBka100();
		}

		function findaaz217InjuryByBka100() {
			var inputFeeInjuryData = $("#inputFeeInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!findaaz217Injury.action",
					inputFeeInjuryData, function(json) {
		...
		function findaaz217Injury() {
			if (window.event.keyCode == 13) {
				...
				postJSON(
						"${rootPath}/inhospital/InhospitalManagerAction!findaaz217Injury.action?inHospitalDTO.querystring="
		...
		function saveFee() {
			...
			var saveItemData = $("#inputFeeInjury").serialize();
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!inputFeeInjury.action",
		...
		function tryCalcTreatInjury() {
			...
			postJSON(
					"${rootPath}/inhospital/InhospitalManagerAction!tryCalcTreatInjury.action?inHospitalDTO.kcd1id="
