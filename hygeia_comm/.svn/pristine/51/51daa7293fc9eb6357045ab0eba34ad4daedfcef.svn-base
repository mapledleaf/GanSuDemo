package com.powersi.biz.medicare.universal.pojo;

import java.math.BigDecimal;
import java.util.Date;

import com.powersi.comm.mybatis.Page;

public class KCD2_Fee_HisDTO extends Page{
	private static final long serialVersionUID = 1L;
	private String kcd2id;    //人员医疗费用明细ID
	private String kcd1id;    //结算云的费用序列号
	private String akc190;    //住院号
	private String akb020;    //医院编号
	private String aaz217;    //就医登记号
	private    int bka001;       //费用批次
	private    int aaz213;       //费用序列号
	private String aaa027;    //统筹区编码（分区用）
	private   Date akc194;      //结算时间（分区用）
	private String ake001;    //社保三大目录编码(对应kae8.bkc144)
	private String ake002;    //社保三大目录名称(对应kae8.bkc143)
	private    int aaz277;       //医疗机构三大目录ID(暂不确定)
	private    int aaz231;       //社保三大目录ID(暂不确定)
	private String ake105;    //药监局药品编码(暂不确定)
	private String bka511;    //城职对应待遇值（自付比例支付类型）
	private String ake005;    //医疗机构三大目录编码
	private String ake006;    //医疗机构三大目录名称
	private String ake003;    //三大目录类别(码表 AKE003)
	private String aka063;    //医疗发票项目类别（码表 AKA063）
	private String aka065;    //收费项目等级(甲、乙、丙)
	private String bka071;    //医疗机构序列号
	private String aka070;    //剂型
	private String aka067;    //药品剂量单位
	private String aka074;    //规格
	private String bka073;    //厂家
	private    int akc226;       //数量
	private BigDecimal bka040;//标准单价
	private BigDecimal aae019;//金额
	private String bkz103;    //限制用药标识
	private String bka067;    //费用冻结标志，用来表识参保人所在单位的基本医疗保险被冻结期间录入的费用。0：未冻结；1：已冻结；2：冻结已处理
	private    int bka062;       //退费对应的费用序列号
	private BigDecimal bka059;//退费金额
	private   Date bka069;      //费用上传时间
	private   Date ake007;      //费用发生时间
	private String bka063;    //录入人工号
	private String bka064;    //录入人名称
	private String bka065;    //录入时间
	private String bka070;    //处方号
	private String bka074;    //处方医师编号
	private String bka075;    //处方医师名称
	private    int bkh015;       //费用套餐id
	private BigDecimal bka080;//优惠金
	private String bkc194;//匹配id
	private String bkm001;    //是否在岗医师标识：0，非在岗；1，在岗
	private String zxxmdj;
	private String bz1;
	private String bz2;
	private String bz3;
	private String drbz;
	private Long bkc109;//异名id
	private double aka057;//先自付比例
	private double bkc106;//自付比例
	
	
	
	public double getBkc106() {
		return bkc106;
	}
	public void setBkc106(double bkc106) {
		this.bkc106 = bkc106;
	}
	public double getAka057() {
		return aka057;
	}
	public void setAka057(double aka057) {
		this.aka057 = aka057;
	}
	public Long getBkc109() {
		return bkc109;
	}
	public void setBkc109(Long bkc109) {
		this.bkc109 = bkc109;
	}
	public String getBkc194() {
		return bkc194;
	}
	public void setBkc194(String bkc194) {
		this.bkc194 = bkc194;
	}
	public String getKcd2id() {
		return kcd2id;
	}
	public void setKcd2id(String kcd2id) {
		this.kcd2id = kcd2id;
	}
	public String getKcd1id() {
		return kcd1id;
	}
	public void setKcd1id(String kcd1id) {
		this.kcd1id = kcd1id;
	}
	public String getAkc190() {
		return akc190;
	}
	public void setAkc190(String akc190) {
		this.akc190 = akc190;
	}
	public String getAkb020() {
		return akb020;
	}
	public void setAkb020(String akb020) {
		this.akb020 = akb020;
	}
	public String getAaz217() {
		return aaz217;
	}
	public void setAaz217(String aaz217) {
		this.aaz217 = aaz217;
	}
	public int getBka001() {
		return bka001;
	}
	public void setBka001(int bka001) {
		this.bka001 = bka001;
	}
	public int getAaz213() {
		return aaz213;
	}
	public void setAaz213(int aaz213) {
		this.aaz213 = aaz213;
	}
	public String getAaa027() {
		return aaa027;
	}
	public void setAaa027(String aaa027) {
		this.aaa027 = aaa027;
	}
	public Date getAkc194() {
		return akc194;
	}
	public void setAkc194(Date akc194) {
		this.akc194 = akc194;
	}
	public String getAke001() {
		return ake001;
	}
	public void setAke001(String ake001) {
		this.ake001 = ake001;
	}
	public String getAke002() {
		return ake002;
	}
	public void setAke002(String ake002) {
		this.ake002 = ake002;
	}
	public int getAaz277() {
		return aaz277;
	}
	public void setAaz277(int aaz277) {
		this.aaz277 = aaz277;
	}
	public int getAaz231() {
		return aaz231;
	}
	public void setAaz231(int aaz231) {
		this.aaz231 = aaz231;
	}
	public String getAke105() {
		return ake105;
	}
	public void setAke105(String ake105) {
		this.ake105 = ake105;
	}
	public String getBka511() {
		return bka511;
	}
	public void setBka511(String bka511) {
		this.bka511 = bka511;
	}
	public String getAke005() {
		return ake005;
	}
	public void setAke005(String ake005) {
		this.ake005 = ake005;
	}
	public String getAke006() {
		return ake006;
	}
	public void setAke006(String ake006) {
		this.ake006 = ake006;
	}
	public String getAke003() {
		return ake003;
	}
	public void setAke003(String ake003) {
		this.ake003 = ake003;
	}
	public String getAka063() {
		return aka063;
	}
	public void setAka063(String aka063) {
		this.aka063 = aka063;
	}
	public String getAka065() {
		return aka065;
	}
	public void setAka065(String aka065) {
		this.aka065 = aka065;
	}
	public String getBka071() {
		return bka071;
	}
	public void setBka071(String bka071) {
		this.bka071 = bka071;
	}
	public String getAka070() {
		return aka070;
	}
	public void setAka070(String aka070) {
		this.aka070 = aka070;
	}
	public String getAka067() {
		return aka067;
	}
	public void setAka067(String aka067) {
		this.aka067 = aka067;
	}
	public String getAka074() {
		return aka074;
	}
	public void setAka074(String aka074) {
		this.aka074 = aka074;
	}
	public String getBka073() {
		return bka073;
	}
	public void setBka073(String bka073) {
		this.bka073 = bka073;
	}
	public int getAkc226() {
		return akc226;
	}
	public void setAkc226(int akc226) {
		this.akc226 = akc226;
	}
	public BigDecimal getBka040() {
		return bka040;
	}
	public void setBka040(BigDecimal bka040) {
		this.bka040 = bka040;
	}
	public BigDecimal getAae019() {
		return aae019;
	}
	public void setAae019(BigDecimal aae019) {
		this.aae019 = aae019;
	}
	public String getBkz103() {
		return bkz103;
	}
	public void setBkz103(String bkz103) {
		this.bkz103 = bkz103;
	}
	public String getBka067() {
		return bka067;
	}
	public void setBka067(String bka067) {
		this.bka067 = bka067;
	}
	public int getBka062() {
		return bka062;
	}
	public void setBka062(int bka062) {
		this.bka062 = bka062;
	}
	public BigDecimal getBka059() {
		return bka059;
	}
	public void setBka059(BigDecimal bka059) {
		this.bka059 = bka059;
	}
	public Date getBka069() {
		return bka069;
	}
	public void setBka069(Date bka069) {
		this.bka069 = bka069;
	}
	public Date getAke007() {
		return ake007;
	}
	public void setAke007(Date ake007) {
		this.ake007 = ake007;
	}
	public String getBka063() {
		return bka063;
	}
	public void setBka063(String bka063) {
		this.bka063 = bka063;
	}
	public String getBka064() {
		return bka064;
	}
	public void setBka064(String bka064) {
		this.bka064 = bka064;
	}
	public String getBka065() {
		return bka065;
	}
	public void setBka065(String bka065) {
		this.bka065 = bka065;
	}
	public String getBka070() {
		return bka070;
	}
	public void setBka070(String bka070) {
		this.bka070 = bka070;
	}
	public String getBka074() {
		return bka074;
	}
	public void setBka074(String bka074) {
		this.bka074 = bka074;
	}
	public String getBka075() {
		return bka075;
	}
	public void setBka075(String bka075) {
		this.bka075 = bka075;
	}
	public int getBkh015() {
		return bkh015;
	}
	public void setBkh015(int bkh015) {
		this.bkh015 = bkh015;
	}
	public BigDecimal getBka080() {
		return bka080;
	}
	public void setBka080(BigDecimal bka080) {
		this.bka080 = bka080;
	}
	public String getBkm001() {
		return bkm001;
	}
	public void setBkm001(String bkm001) {
		this.bkm001 = bkm001;
	}
	public String getZxxmdj() {
		return zxxmdj;
	}
	public void setZxxmdj(String zxxmdj) {
		this.zxxmdj = zxxmdj;
	}
	public String getBz1() {
		return bz1;
	}
	public void setBz1(String bz1) {
		this.bz1 = bz1;
	}
	public String getBz2() {
		return bz2;
	}
	public void setBz2(String bz2) {
		this.bz2 = bz2;
	}
	public String getBz3() {
		return bz3;
	}
	public void setBz3(String bz3) {
		this.bz3 = bz3;
	}
	public String getDrbz() {
		return drbz;
	}
	public void setDrbz(String drbz) {
		this.drbz = drbz;
	}
}
